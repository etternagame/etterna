...
t[#t + 1] =
	...
	InitCommand = function(self)
		self:xy(infoboxx, infoboxy):zoomto(infoboxwidth, infoboxheight):halign(0):valign(0):diffuse(color("#333333CC")):diffusealpha(
			0.66
		)
	end
}
t[#t + 1] =
	Def.Quad {
	InitCommand = function(self)
		self:xy(infoboxx, infoboxy):zoomto(infoboxbar, infoboxheight):halign(0):valign(0):diffuse(getMainColor("highlight")):diffusealpha(
			0.5
		)
	end
}
...
t[#t + 1] =
	...
	{
		InitCommand = function(self)
			self:xy(lengthx, lengthy):visible(true):halign(1):zoom(capWideScale(get43size(0.4), 0.4)):maxwidth(
