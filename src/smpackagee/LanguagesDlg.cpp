...
void
LanguagesDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST_THEMES, m_listThemes);
	DDX_Control(pDX, IDC_LIST_LANGUAGES, m_listLanguages);
	DDX_Control(pDX,
				IDC_CHECK_EXPORT_ALREADY_TRANSLATED,
				m_buttonExportAlreadyTranslated);
}

BOOL
LanguagesDlg::OnInitDialog()
{
...
static RString
GetCurrentString(const CListBox& list)
{
...
static void
SelectString(CListBox& list, const RString& sToSelect)
{
...
void
LanguagesDlg::OnSelchangeListThemes()
{
	// TODO: Add your control notification handler code here
	...
	if (!sTheme.empty()) {
		RString sLanguagesDir = SpecialFiles::THEMES_DIR + sTheme + "/" +
								SpecialFiles::LANGUAGES_SUBDIR;
		...
		FOREACH_CONST(RString, vs, s)
		{
			...
			RString sLanguage =
			  SMPackageUtil::GetLanguageDisplayString(sIsoCode);
...
static RString
GetLanguageFile(const RString& sTheme, const RString& sLanguage)
{
	return SpecialFiles::THEMES_DIR + sTheme + "/" +
		   SpecialFiles::LANGUAGES_SUBDIR + sLanguage + ".ini";
}

static int
GetNumValuesInIniFile(const RString& sIniFile)
{
	int count = 0;
	IniFile ini;
	ini.ReadFile(sIniFile);
	FOREACH_CONST_Child(&ini, key) { FOREACH_CONST_Attr(key, value) count++; }
	return count;
}

static int
GetNumIntersectingIniValues(const RString& sIniFile1, const RString& sIniFile2)
{
	int count = 0;
	IniFile ini1;
...
void
LanguagesDlg::OnSelchangeListLanguages()
{
	// TODO: Add your control notification handler code here
	int iTotalStrings = -1;
	...
	if (!sTheme.empty()) {
		RString sBaseLanguageFile =
		  GetLanguageFile(sTheme, SpecialFiles::BASE_LANGUAGE);
		iTotalStrings = GetNumValuesInIniFile(sBaseLanguageFile);

		if (!sLanguage.empty()) {
			sLanguage =
			  SMPackageUtil::GetLanguageCodeFromDisplayString(sLanguage);

			RString sLanguageFile = GetLanguageFile(sTheme, sLanguage);
			iNeedTranslation =
			  iTotalStrings -
			  GetNumIntersectingIniValues(sBaseLanguageFile, sLanguageFile);
		}
	}

	GetDlgItem(IDC_STATIC_TOTAL_STRINGS)
	  ->SetWindowText(ssprintf(iTotalStrings == -1 ? "" : "%d", iTotalStrings));
	GetDlgItem(IDC_STATIC_NEED_TRANSLATION)
	  ->SetWindowText(
		ssprintf(iNeedTranslation == -1 ? "" : "%d", iNeedTranslation));
	...
	GetDlgItem(IDC_CHECK_EXPORT_ALREADY_TRANSLATED)
	  ->EnableWindow(!sLanguage.empty());
}
...
void
LanguagesDlg::OnBnClickedButtonCreate()
{
	// TODO: Add your control notification handler code here
	...
	RString sLanguageFile =
	  GetLanguageFile(sTheme, RString(dlg.m_sChosenLanguageCode));
	...
	OnSelchangeListThemes();
	SelectString(m_listLanguages,
				 SMPackageUtil::GetLanguageDisplayString(
				   RString(dlg.m_sChosenLanguageCode)));
	OnSelchangeListLanguages();
}

static LocalizedString THIS_WILL_PERMANENTLY_DELETE(
  "LanguagesDlg",
  "This will permanently delete '%s'. Continue?");
void
LanguagesDlg::OnBnClickedButtonDelete()
{
	// TODO: Add your control notification handler code here
	RString sTheme = GetCurrentString(m_listThemes);
	ASSERT(!sTheme.empty());
	...
	RString sMessage =
	  ssprintf(THIS_WILL_PERMANENTLY_DELETE.GetValue(), sLanguageFile.c_str());
...
static LocalizedString THERE_ARE_NO_STRINGS_TO_EXPORT(
  "LanguagesDlg",
  "There are no strings to export for this language.");
static LocalizedString EXPORTED_TO_YOUR_DESKTOP(
  "LanguagesDlg",
  "Exported to your Desktop as '%s'.");
void
LanguagesDlg::OnBnClickedButtonExport()
{
	// TODO: Add your control notification handler code here
	RString sTheme = GetCurrentString(m_listThemes);
	ASSERT(!sTheme.empty());
	...
	bool bExportAlreadyTranslated =
	  !!m_buttonExportAlreadyTranslated.GetCheck();
	...
	if (csv.WriteFile(file))
		Dialog::OK(
		  ssprintf(EXPORTED_TO_YOUR_DESKTOP.GetValue(), sFile.c_str()));
	else
		Dialog::OK(ssprintf(FAILED_TO_SAVE.GetValue(), sFullFile.c_str()));
}

static LocalizedString ERROR_READING_FILE("LanguagesDlg",
										  "Error reading file '%s'.");
static LocalizedString ERROR_PARSING_FILE("LanguagesDlg",
										  "Error parsing file '%s'.");
static LocalizedString IMPORTING_THESE_STRINGS_WILL_OVERRIDE(
  "LanguagesDlg",
  "Importing these strings will override all data in '%s'. Continue?");
static LocalizedString ERROR_READING("LanguagesDlg", "Error reading '%s'.");
static LocalizedString ERROR_READING_EACH_LINE_MUST_HAVE(
  "LanguagesDlg",
  "Error reading line '%s' from '%s': Each line must have either 3 or 4 "
  "values.  This row has %d values.");
static LocalizedString IMPORTED_STRINGS_INTO("LanguagesDlg",
											 "Imported %d strings into '%s'.\n "
											 "- %d were added.\n - %d were "
											 "modified\n - %d were unchanged\n "
											 "- %d empty strings were ignored");
void
LanguagesDlg::OnBnClickedButtonImport()
{
	// TODO: Add your control notification handler code here
	CFileDialog dialog(TRUE, // file open?
	...
	{
		Dialog::Result ret = Dialog::AbortRetry(
		  ssprintf(IMPORTING_THESE_STRINGS_WILL_OVERRIDE.GetValue(),
				   sLanguageFile.c_str()));
		if (ret != Dialog::retry)
			return;
	}
	...
	FOREACH_CONST(CsvFile::StringVector, csv.m_vvs, line)
	{
		...
		if (tl.sCurrentLanguage.empty()) {
			...
			bool bExists =
			  ini.GetValue(tl.sSection, tl.sID, sOldCurrentLanguage);
			if (bExists) {
	...
	if (ini.WriteFile(sLanguageFile))
		Dialog::OK(ssprintf(IMPORTED_STRINGS_INTO.GetValue(),
							iNumImported,
							sLanguageFile.c_str(),
							iNumAdded,
							iNumModified,
...
void
GetAllMatches(const RString& sRegex,
			  const RString& sString,
...
void
DumpList(const vector<RString>& asList, RageFile& file)
{
	RString sLine;
...
void
LanguagesDlg::OnBnClickedCheckLanguage()
{
	RString sTheme = GetCurrentString(m_listThemes);
	ASSERT(!sTheme.empty());
	RString sLanguage = GetCurrentString(m_listLanguages);
	...
	{ FOREACH_CONST_Child(&ini1, key){
	  FOREACH_CONST_Attr(key, value){ const RString& sSection = key->m_sName;
	...
	/* Check &FOO;, %{foo}, %-1.2f, %.  Some mismatches here are normal,
	 * particularly in languages with substantially different word order rules
	 * than English. */
	{
		RString sRegex =
		  "(&[A-Za-z]+;|%{[A-Za-z]+}|%[+-]?[0-9]*.?[0-9]*[sidf]|%)";
		...
		if (asMatchesBase.size() != asMatches.size() ||
			!std::equal(
			  asMatchesBase.begin(), asMatchesBase.end(), asMatches.begin())) {
			file.PutLine(
			  ssprintf("Entity/substitution mismatch in section [%s] (%s):",
					   sSection.c_str(),
					   sID.c_str()));
	...
	/* Check "foo::bar", "foo\nbar" and double quotes.  Check these
	 * independently of the above. */
	{
		...
		if (asMatchesBase.size() != asMatches.size() ||
			!std::equal(
			  asMatchesBase.begin(), asMatchesBase.end(), asMatches.begin())) {
			file.PutLine(
			  ssprintf("Quote/line break mismatch in section [%s] (%s):",
					   sSection.c_str(),
					   sID.c_str()));
			file.PutLine(ssprintf("    %s", sCurrentLanguage.c_str()));
			file.PutLine(ssprintf("    %s", sBaseLanguage.c_str()));
	...
	/* Check that both end in a period or both don't end an a period. */
	{
		...
		if ((sBaseLanguage2.Right(1) == ".") ^
			(sCurrentLanguage2.Right(1) == ".")) {
			file.PutLine(ssprintf("Period mismatch in section [%s] (%s):",
								  sSection.c_str(),
								  sID.c_str()));
			file.PutLine(ssprintf("    %s", sCurrentLanguage.c_str()));
			file.PutLine(ssprintf("    %s", sBaseLanguage.c_str()));
...
/* The text isn't valid UTF-8.  We don't know what encoding it is; guess that
 * it's ISO-8859-1 and convert it so it'll display correctly and can be pasted
 * into the file.  Don't include the original text: if we include multiple
 * encodings in the resulting text file, editors won't understand the encoding
 * of the file and will pick one arbitrarily. */
...
{ FOREACH_CONST_Child(&ini2, key){ vector<RString> asList;
...
if (!asList.empty()) {
	file.PutLine(
	  ssprintf("Obsolete translation in section [%s]:", sSection.c_str()));
...

{
	FOREACH_CONST_Child(&ini1, key)
	{
		...
		if (!asList.empty()) {
			file.PutLine(
			  ssprintf("Not translated in section [%s]:", sSection.c_str()));
			DumpList(asList, file);
		}
	}
}
