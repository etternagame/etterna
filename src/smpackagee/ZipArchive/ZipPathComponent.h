 ...
 */
#if !defined(                                                                  \
  AFX_ZIPPATHCOMPONENT_H__9B222C08_AD11_4138_96CC_1237511E3E37__INCLUDED_)
...
class ZIP_API CZipPathComponent
{
  public:
	...
	static const TCHAR m_cSeparator; ///< A system - specific default path
									 ///< separator. Defined in ZipPlatform.cpp.
									 /**
	...
	*/
	 ...
	 */
	static bool IsSeparator(TCHAR c) { return c == _T('\\') || c == _T('/'); }
	...
	*/
	...
	\see SetFullPath
*/
	CZipPathComponent(LPCTSTR lpszFullPath) { SetFullPath(lpszFullPath); }
...
*/
  ...
  protected:
	...
	*/
	//@{
	CZipString m_szDirectory, ///< a directory(ies) (one or more) without the
							  ///< path separators at the end and the beginning
	  m_szFileTitle,		  ///< a filename without an extension
	  m_szFileExt,			  ///< a file extension without a dot
	  m_szDrive,  ///< a drive (if the system path standard uses it) without a
				  ///< path separator at the end
	  m_szPrefix; ///< a prefix (e.g. for the UNC path or Unicode path under
				  ///< Windows)
				  //@}
};
