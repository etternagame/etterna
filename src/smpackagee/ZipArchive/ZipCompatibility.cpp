...
// more to come...
conv_func conv_funcs[11] = { AttrDos, NULL,	NULL, AttrUnix, NULL,   NULL,
							 AttrDos, AttrMac, NULL, NULL,	 AttrDos };

DWORD
ZipCompatibility::ConvertToSystem(DWORD uAttr, int iFromSystem, int iToSystem)
{
...
DWORD
AttrDos(DWORD uAttr, bool)
{
	return uAttr;
}

DWORD
AttrUnix(DWORD uAttr, bool bFrom)
{
	DWORD uNewAttr = 0;
	if (bFrom) {
		...
		// we may set archive attribute if the file hasn't the execute
		// permissions
		//
		if (!(uAttr & 1))
			uNewAttr |= attArch;
		...
		uNewAttr = 0; // we cannot assume that if the file hasn't the archive
					  // attribute set
		...
		if (!(uAttr & attHidd))
			uNewAttr |=
			  (CREATE_OTHER_PERMISSIONS(4) | CREATE_GROUP_PERMISSIONS(4)) |
			  CREATE_USER_PERMISSIONS(4);

		if (!(uAttr & attROnly))
			uNewAttr |=
			  (CREATE_GROUP_PERMISSIONS(2) | CREATE_USER_PERMISSIONS(2));
		if (uAttr & attDir)
...
// ************************************************************************
ZIPINLINE bool
ZipCompatibility::IsPlatformSupported(int iCode)
{
...
void
ZipCompatibility::FileNameUpdate(CZipFileHeader& header, bool bFromZip)
{
...
void
ZipCompatibility::SlashBackslashChg(CZipAutoBuffer& buffer, bool bReplaceSlash)
{
