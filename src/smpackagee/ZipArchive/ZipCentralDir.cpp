...
CZipCentralDir::Read()
{
	...
	// if m_uThisDisk is not zero, it is enough to say that it is a multi disk
	// archive
	ASSERT((!m_info.m_uThisDisk &&
			(m_info.m_uEntriesNumber == m_info.m_uDiskEntriesNo) &&
			!m_info.m_uDiskWithCD) ||
		   m_info.m_uThisDisk);
...
void
CZipCentralDir::ThrowError(int err) const
{
...
{
	m_pStorage->m_pFile->Seek(m_info.m_uOffset + m_info.m_uBytesBeforeZip,
	...
	SortHeaders(); // this is necessary when deleting files and removing data
				   // descriptors
...
CZipCentralDir::SortHeaders()
{
	...
	if (iSize)
		qsort((void*)&(m_headers[0]),
			  iSize,
			  sizeof(CZipFileHeader*),
			  CompareHeaders);
}

void
CZipCentralDir::Clear(bool bEverything)
{
	m_pOpenedFile = NULL;
...
bool
CZipCentralDir::IsValidIndex(int uIndex) const
{
	return uIndex < m_headers.GetSize() && uIndex >= 0;
}

void
CZipCentralDir::OpenFile(WORD uIndex)
{
	WORD uLocalExtraFieldSize;
	m_pOpenedFile = (*this)[uIndex];
	m_pStorage->ChangeDisk(m_pOpenedFile->m_uDiskStart);
	m_pStorage->m_pFile->Seek(m_pOpenedFile->m_uOffset +
								m_info.m_uBytesBeforeZip,
							  CZipAbstractFile::begin);
...
void
CZipCentralDir::CloseFile(bool bAfterException)
{
	if (!m_pOpenedFile)
		return;
	m_pLocalExtraField.Release();
	if (!bAfterException && m_pOpenedFile->IsDataDescr()) {
		...
		// in span mode, files that are divided between disks have bit 3 of flag
		// set which tell about the presence of the data descriptor after the
		// compressed data This signature may be in the disk spanning archive
		// that is one volume only (it is detected as a non disk spanning
		// archive)
		if (memcmp(buf, CZipStorage::m_gszExtHeaderSignat, 4) !=
			0) // there is no signature
...
CZipFileHeader*
CZipCentralDir::AddNewFile(const CZipFileHeader& header, int iReplaceIndex)
{
	...
	if (m_bFindFastEnabled)
		InsertFindFastElement(
		  pHeader, uIndex); // GetCount > 0, 'cos we've just added a header
	RemoveFromDisk();
	if (bReplace)
...
void
CZipCentralDir::RemoveFromDisk()
{
	if (m_info.m_bOnDisk) {
		ASSERT(!m_pStorage->IsSpanMode()); // you can't add files to the
										   // existing disk span archive or to
										   // delete them from it
		m_pStorage->m_pFile->SetLength(m_info.m_uBytesBeforeZip +
									   m_info.m_uOffset);
		m_info.m_bOnDisk = false;
	} else
		m_pStorage->Flush(); // if remove from disk is requested, then the
							 // archive modification will follow, so flush the
							 // buffers
}
...
CZipCentralDir::CloseNewFile()
{
	...
	} else /*if (!IsSpan)*/
	{
		...
		m_pStorage->Flush();
		// the offset contains bytes before zip (set while writting the local
		// header)
		m_pStorage->m_pFile->Seek(m_pOpenedFile->m_uOffset + 14,
								  CZipAbstractFile::begin);
		// we don't have to restore the pointer, because before adding a new
		// file, the pointer is moved to the end
	}
	...
	if (!bIsSpan) {
		if (bEncrypted) {
			...
			m_pStorage->Flush();
			m_pStorage->m_pFile->Seek(m_info.m_uBytesBeforeZip +
										m_pOpenedFile->m_uOffset + 14,
									  CZipAbstractFile::begin);
...
void
CZipCentralDir::Write(CZipActionCallback* pCallback)
{
	if (m_info.m_bOnDisk)
		return;
	...
	// if there is a disk spanning archive in creation and it is only
	// one-volume,
	...
	if (m_pStorage->IsSpanMode() && m_pStorage->GetCurrentDisk() == 0) {
		...
		// calculate the size of data descriptors already in the buffer or on
		// the disk (they will be removed in the non disk spanning archive):
		...
		DWORD uToGrow =
		  GetSize(true) - (4 + m_info.m_uEntriesNumber * (4 + 12));
		...
		// lets make sure it will be one-disk archive
		{
			...
			if (!m_pStorage->m_iBytesWritten && // no bytes on the disk yet
				(m_pStorage->GetFreeInBuffer() >=
				 uToGrow)) // is the buffer big enough?
			{
				RemoveDataDescr(true);
				bDontAllowDiskChange = true; // if the disk change occurs
											 // somehow, we'll throw an error
											 // later
			} else {
				m_pStorage->Flush();
				if (RemoveDataDescr(false))
					bDontAllowDiskChange = true; // if the disk change occurs
												 // somehow, we'll throw an
												 // error later
			}
		}
	}
	...
	try {
		WriteHeaders(pCallback,
					 bDontAllowDiskChange || !m_pStorage->IsSpanMode());
...
void
CZipCentralDir::WriteHeaders(CZipActionCallback* pCallback, bool bOneDisk)
{
	...
	for (int i = 0; i < m_info.m_uEntriesNumber; i++) {
		...
		if (m_bConvertAfterOpen)
			// if CZipArchive::Flush is called we will be still using the
			// archive, so restore changed name
			...
			if (!pCallback->Callback(iStep)) {
				...
				if (bOneDisk) {
					if (!m_pStorage->IsSpanMode())
						...
						m_pStorage->Flush(); // must be flush before - flush was
											 // not called in span mode
...
DWORD
CZipCentralDir::WriteCentralEnd()
{
	DWORD uSize = GetSize();
...
void
CZipCentralDir::RemoveFile(CZipFileHeader* pHeader, int iIndex, bool bShift)
{
...
DWORD
CZipCentralDir::GetSize(bool bWhole) const
{
	DWORD uHeaders = 0;
	int iCount = m_headers.GetSize();
	if (bWhole) {
...
bool
CZipCentralDir::RemoveDataDescr(bool bFromBuffer)
{
	...
	for (int i = 0; i < iCount; i++) {
		...
		if (!pHeader->IsEncrypted()) {
			...
			uExtraHeaderLen =
			  4 /*ext. header signature*/ + 12 /*data descriptor*/;
		} else
		...
		DWORD uToCopy =
		  (i == (iCount - 1) ? uSize : m_headers[i + 1]->m_uOffset) -
...
void
CZipCentralDir::BuildFindFastArray(bool bCaseSensitive)
{
	m_findarray.RemoveAll();
	...
	if (!m_bConvertAfterOpen) {
		for (int i = 0; i < iCount; i++) {
			...
			InsertFindFastElement(
			  &fh, i); // this method requires the name to be already converted
		}
	} else
		for (int i = 0; i < iCount; i++)
...
void
CZipCentralDir::EnableFindFast(bool bEnable, bool bCaseSensitive)
{
...
int
CZipCentralDir::FindFile(LPCTSTR lpszFileName,
						 bool bCaseSensitive,
						 bool bSporadically,
						 bool bFileNameOnly)
{
	...
	if (!m_bConvertAfterOpen) {
		TRACE(
		  _T("%s(%i) : Converting all the filenames.\n"), __FILE__, __LINE__);
		ConvertAll();
	}
	if (!m_bFindFastEnabled)
...
void
CZipCentralDir::InsertFindFastElement(CZipFileHeader* pHeader, WORD uIndex)
{
	CZipString fileName = pHeader->GetFileName();
	int iSize = m_findarray.GetSize();

	//	Our initial binary search range encompasses the entire array of
	// filenames:
	int start = 0;
	int end = iSize;
	...
	while (start < end) {
		...
		//	Compare the filename with the filename at the midpoint of the
		// current search range:
		int result = CompareElement(fileName, (WORD)midpoint);

		//	If our filename is larger, it must fall in the first half of the
		// search range:
		if (result > 0) {
			end = midpoint;
		}
	...
	m_findarray.InsertAt(
	  start,
	  CZipFindFast(
		pHeader, WORD(uIndex == WORD(-1) ? iSize : uIndex /* just in case */)));
}

int
CZipCentralDir::FindFileNameIndex(LPCTSTR lpszFileName) const
{
	int start = 0;
	...
	while (start <= end) {
		...
		//	Compare the given filename with the filename at the midpoint of the
		// search range:
		int result = CompareElement(lpszFileName, (WORD)midpoint);

		//	If our filename is smaller, it must fall in the first half of the
		// search range:
		if (result > 0) {
			end = midpoint - 1;
		}
...
void
CZipCentralDir::RenameFile(WORD uIndex, LPCTSTR lpszNewName)
{
