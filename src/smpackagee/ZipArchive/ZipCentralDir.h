  ...
  public:
	...
	*/
	struct ZIP_API CZipFindFast
	{
		...
		/**
				A pointer to the structure in CZipCentralDir. We extract a name
		   from it.
			*/
	...
	*/
	struct ZIP_API Info
	{
		DWORD m_uCentrDirPos; ///< the position of the beginning of the central
							  ///< directory (as located by #Locate)
		DWORD m_uBytesBeforeZip; ///< The number of bytes before the actual zip
								 ///< archive in a file. It is non-zero for
								 ///< self-extracting archives.
		WORD m_uThisDisk; ///< number of the disk with the central directory end
						  ///< record (the number of disks in the disk-spanning
						  ///< archive)
		WORD m_uDiskWithCD;	///< number of the disk with the start of the
							   ///< central directory
		WORD m_uDiskEntriesNo; ///< total number of entries in the central dir
							   ///< on this disk
		WORD m_uEntriesNumber; ///< total number of entries in the central dir
		DWORD m_uSize;		   ///< size of the central directory (valid only if
							   ///< #m_bOnDisk is \c true; use #GetSize instead)
		DWORD m_uOffset; ///< offset of start of central directory with respect
						 ///< to the starting disk number (as written in the
						 ///< central directory record); valid only if
						 ///< #m_bOnDisk is \c true
		bool m_bOnDisk;
		//\c true if the central directory is physically present in the archive
	  ...
	  protected:
		...
		{
			m_uBytesBeforeZip =
			  bIsSpan ? 0 : m_uCentrDirPos - m_uSize - m_uOffset;
		}
		bool CheckIfOK_2()
		{
			return (m_uSize || !m_uEntriesNumber) &&
				   (m_uEntriesNumber || !m_uSize);
		}
	...
	char
	  m_szSignature[4]; ///< end of central dir signature (must be 0x06054b50)

	CZipAutoBuffer m_pszComment;	   ///< the archive comment
	CZipAutoBuffer m_pLocalExtraField; ///< a local extra field
	CZipFileHeader* m_pOpenedFile;	 ///< points to a currently opened file or
									   ///< NULL if no file is opened
	...
	*/
	...
	\param	uIndex
		a zero-based index
	\return	\c true if the file with the given index exists inside the
	archive; otherwise \c false;
*/
	...
	\note Throws exceptions.
*/
	void RemoveFile(CZipFileHeader* pHeader,
					int iIndex = -1,
					bool bShift = true);
	...
	*/
	...
	\param	bEverything
		- \c true - clear some attributes and remove all the file headers from
	memory
...
*/
	...
	\param iReplaceIndex if different from -1, the index of the file to be
	replaced \return the pointer to the new header \note Throws exceptions.
*/
...
*/
	...
	*/
	...
	/**
		\see CZipArchive::FindFile
		\note \e bSporadically set to \c false rebuilds #m_findarray if
	   necessary
	*/
	...
	*/
	...
	CZipFileHeader* operator[](int iIndex) { return m_headers[iIndex]; }
	...
	/**
		...
		the archive.
		- If \c false, the conversion takes place on each call to
	   CZipArchive::GetFileInfo

		Change is value with CZipArchive::SetConvertAfterOpen.

		Set it to \c true when you plan to use CZipArchive::FindFile or get the
	   stored files information. <BR> Set it to \c false when you plan mostly to
	   modify the archive.
...
*/
	void ConvertFileName(bool bFromZip,
						 bool bAfterOpen,
						 CZipFileHeader* pHeader = NULL) const
	{
...
*/
	...
	\return	the index in #m_findarray with the appropriate CZipFindFast
	structure or \c -1 if there is no file with the given name \see
...
*/
	...
	{
		return (m_findarray[uIndex].m_pHeader->GetFileName().*
				m_pCompare)(lpszFileName);
	}
	/**
...
*/
	...
	<B><CODE> typedef int (CZipString::*ZIPSTRINGCOMPARE)( LPCTSTR ) const;
	</CODE></B>
	*/
	...
	*/
	...
	/**
		\return the location of the beginning of the central dir end record in
	   the archive \note Throws exceptions.
	*/
	DWORD Locate();
	/**
	...
	*/
	...
	\param	bFromBuffer
		if \c true, remove from the buffer in memory otherwise from the file on
	a disk \return	\c false if the file mapping to memory was not successful
