...
int
CZipException::ZlibErrToZip(int iZlibError)
{
	switch (iZlibError) {
		case 2: // Z_NEED_DICT:
...
BOOL
CZipException::GetErrorMessage(LPTSTR lpszError, UINT nMaxError, UINT*)
...
CZipString
CZipException::GetInternalErrorDescription(int iCause, bool bNoLoop)
{
	CZipString sz;
	switch (iCause) {
		case EROFS:
		...
		case abortedSafely:
			...
			break;
		case nonRemovable:
			sz = _T("The device selected for the disk spanning archive is non ")
				 _T("removable");
			break;
		case tooManyVolumes:
		...
		case tooLongFileName:
			sz = _T("The filename of the file being added to the archive is ")
				 _T("too long");
			break;
		case badPassword:
			...
			break;
		case dirWithSize:
			sz = _T("During testing found the directory with the size greater ")
				 _T("than 0");
			break;
		case internal:
		...
		case notRemoved:
			sz.Format(_T("%s (%s)"),
					  _T("Error while removing a file"),
					  (LPCTSTR)GetSystemErrorDescription());
			break;
		case notRenamed:
			sz.Format(_T("%s (%s)"),
					  _T("Error while renaming a file"),
					  (LPCTSTR)GetSystemErrorDescription());
			break;
		case platfNotSupp:
			...
			break;
		case cdirNotFound:
			sz = _T("The central directory was not found in the archive (or ")
				 _T("you were trying to open not the last disk of a ")
				 _T("multi-disk archive)");
			break;
		case streamEnd:
