...
public:
	...
	void TotalMaxPoints();		// Counts up the total points and assigns it
								...
								starting iteration. */
	float Chisel(float pskill, float res, int iter, bool stam, bool jack, bool nps, bool js, bool hs, bool jackstam);

	vector<float> OHJumpDownscaler(const vector<NoteInfo>& NoteInfo, int t1, int t2);
	vector<float> Anchorscaler(const vector<NoteInfo>& NoteInfo, int t1, int t2);
