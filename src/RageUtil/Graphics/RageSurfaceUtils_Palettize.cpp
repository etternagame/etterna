...
#define PAM_DEPTH(p)                                                           \
	PAM_ASSIGN((p),                                                            \
			   (uint8_t)table[PAM_GETR(p)],                                    \
			   (uint8_t)table[PAM_GETG(p)],                                    \
			   (uint8_t)table[PAM_GETB(p)],                                    \
			   (uint8_t)table[PAM_GETA(p)])
...
struct acolorhash_hash
{
	...
	~acolorhash_hash()
	{
		for (auto& i : hash) {
			...
			for (achl = i; achl != nullptr; achl = achlnext) {
				achlnext = achl->next;
				free(achl);
			}
		}
...
static acolorhist_item*
mediancut(acolorhist_item* achv,
		  int colors,
		  int sum,
		  int maxval,
		  int newcolors);
...
static bool
compare_index_0(const acolorhist_item& ch1, const acolorhist_item& ch2)
{
	return ch1.acolor[0] < ch2.acolor[0];
}

static bool
compare_index_1(const acolorhist_item& ch1, const acolorhist_item& ch2)
{
	return ch1.acolor[1] < ch2.acolor[1];
}

static bool
compare_index_2(const acolorhist_item& ch1, const acolorhist_item& ch2)
{
	return ch1.acolor[2] < ch2.acolor[2];
}

static bool
compare_index_3(const acolorhist_item& ch1, const acolorhist_item& ch2)
{
	return ch1.acolor[3] < ch2.acolor[3];
}

static acolorhist_item*
pam_computeacolorhist(const RageSurface* src, int maxacolors, int* acolorsP);
static void
pam_addtoacolorhash(acolorhash_hash& acht, const uint8_t acolorP[4], int value);
static int
pam_lookupacolor(const acolorhash_hash& acht, const uint8_t acolorP[4]);
static void
pam_freeacolorhist(acolorhist_item* achv);
...
void
RageSurfaceUtils::Palettize(RageSurface*& pImg, int iColors, bool bDither)
{
	ASSERT(iColors != 0);
...

	{
		...
		acolormap =
		  mediancut(achv, colors, pImg->h * pImg->w, maxval, newcolors);
	...
	for (auto row = 0; row < pImg->h; ++row) {
		...
		do {
			...
			if (bDither) {
				...
				PAM_ASSIGN(pixel,
						   static_cast<uint8_t>(sc[0]),
						   static_cast<uint8_t>(sc[1]),
						   static_cast<uint8_t>(sc[2]),
						   static_cast<uint8_t>(sc[3]));
			}
			...
			if (ind == -1) {
				...
				for (auto i = 0; i < newcolors; ++i) {
					...
					newdist +=
					  pSquareTable[static_cast<int>(pixel[1]) - colors2[1]];
					newdist +=
					  pSquareTable[static_cast<int>(pixel[2]) - colors2[2]];
					newdist +=
					  pSquareTable[static_cast<int>(pixel[3]) - colors2[3]];
...
static bool
CompareBySumDescending(const box& b1, const box& b2)
{
	return b2.sum < b1.sum;
}

static acolorhist_item*
mediancut(acolorhist_item* achv, int colors, int sum, int maxval, int newcolors)
{
	acolorhist_item* acolormap;
	box_vector bv;
	int boxes;

	bv = static_cast<box_vector>(malloc(sizeof(struct box) * newcolors));
	ASSERT(bv != NULL);
	acolormap = static_cast<acolorhist_item*>(
	...
	for (auto bi = 0; bi < boxes; ++bi) {
		...
		PAM_ASSIGN(acolormap[bi].acolor,
...
static bool
pam_computeacolorhash(const RageSurface* src,
					  int maxacolors,
					  int* acolorsP,
		...
		for (auto col = 0; col < src->w; ++col, pP++) {
			...
			if (achl != nullptr)
				...
				if (++(*acolorsP) > maxacolors)
					return false;
				achl = static_cast<acolorhist_list>(
				  malloc(sizeof(struct acolorhist_list_item)));
				ASSERT(achl != NULL);
...
static acolorhist_item*
pam_acolorhashtoacolorhist(const acolorhash_hash& acht, int maxacolors)
{
	// Collate the hash table into a simple acolorhist array.
	auto* achv = static_cast<acolorhist_item*>(
	  malloc(maxacolors * sizeof(struct acolorhist_item)));
	ASSERT(achv != NULL);
