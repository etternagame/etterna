...
static void
WriteBytes(RageFile& f, RString& sError, const void* buf, int size)
{
	if (sError.size() != 0)
		return;
...
static void
write_le16(RageFile& f, RString& sError, uint16_t val)
{
	val = Swap16LE(val);
...
static void
write_le32(RageFile& f, RString& sError, uint32_t val)
{
	val = Swap32LE(val);
