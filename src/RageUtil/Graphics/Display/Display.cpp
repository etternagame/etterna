...
Display::Display::Init(VideoModeParams&& p, bool bAllowUnacceleratedRenderer)
{
	...
	Locator::getLogger()->info(
	  "Current renderer: UnstableDisplay - {}", m_Renderer->GetApiDescription());
...
void
Display::Display::DrawQuadStripInternal(const RageSpriteVertex v[], int iNumVerts)
{
	m_Batcher.InsertCommand(std::source_location::current().function_name());
}
...
void
Display::Display::DrawTrianglesInternal(const RageSpriteVertex v[], int iNumVerts)
{
	m_Batcher.InsertCommand(std::source_location::current().function_name());
}
