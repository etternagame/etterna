...
std::string
hexify(wchar_t const src, unsigned int dstlen);

/** @brief Offer a clean way of replacing all occurances of a character with a
 * different character.
 *
 * See http://stackoverflow.com/a/2896627 */
void
replace(std::string& target, char from, char to);

/** @brief Offer a clean way of replacing all occurances of a string with a
 * different string.
 *
...
std::string
join(std::string const& delimiter, std::vector<std::string> const& source);

/** @brief Join all of the strings together into one string. */
std::string
join(std::string const& delimiter,
...
split(std::string const& source, std::string const& delimiter);
/** @brief Split a string into a vector based on the delimiter. */
std::vector<std::string>
split(std::string const& source,
	  std::string const& delimiter,
	  EmptyEntries shouldSkipEmptyEntries);
/** @brief Split a string into a vector based on the delimiter. */
std::vector<std::wstring>
split(std::wstring const& source, std::wstring const& delimiter);
/** @brief Split a string into a vector based on the delimiter. */
std::vector<std::wstring>
split(std::wstring const& source,
 ...
 * @param start An in-out parameter indicating the starting position of the
 * string to "split" from.
 * @param size An in-out parameter to indicate how far to read in. */
void
split_in_place(std::string const& source,
 ...
 * string to "split" from.
 * @param size An in-out parameter to indicate how far to read in.
 * @param shouldSkipEmptyEntries a flag to indicate if empty elements from the
 * split should be skipped or not. */
void
split_in_place(std::string const& source,
			   std::string const& delimitor,
			   int& start,
 ...
 * @param size An in-out parameter to indicate how far to read in. */
void
split_in_place(std::wstring const& source,
...
std::string
trim_left(std::string const& source);

/** @brief Trim the front of the string of all characters of the caller's
 * choosing. */
std::string
trim_left(std::string const& source, std::string const& delimiters);

/** @brief Trim the back of the string of all whitespace characters. */
std::string
trim_right(std::string const& source);

/** @brief Trim the back of the string of all characters of the caller's
 * choosing. */
std::string
trim_right(std::string const& source, std::string const& delimiters);
...
/** @brief Trim the front and back of the string of all characters of the
 * caller's choosing. */
std::string
trim(std::string const& source, std::string const& delimiters);

/** @brief Retrieve the last named component of a directory/file. */
std::string
base_name(std::string const& dir);

/** @brief Retrieve all except the last named component of a directory/file. */
std::string
dir_name(std::string const& dir);
...
struct ci_ascii_char_traits : public std::char_traits<char>
{
	...
	/** @brief Determine if the first character is less than the other
	 * character. */
...
struct std_string_ci_less
{
	...
	{
		return Rage::ci_ascii_string{ lhs.c_str() } <
