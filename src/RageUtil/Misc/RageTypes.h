...
enum TextureMode {
	TextureMode_Modulate, // Affects one texture stage. Texture is modulated with the diffuse color.
	TextureMode_Glow, /* Affects one texture stage. Color is replaced with white, leaving alpha. Used with BLEND_ADD to add glow. */
	TextureMode_Add, // Affects one texture stage. Color is added to the previous texture stage.
	NUM_TextureMode,
	TextureMode_Invalid
};
LuaDeclareType(TextureMode);

// REMOVE: Most of these effects are unused
enum EffectMode {
	EffectMode_Normal,
...
struct RageMatrix {
  public:
	...
	RageMatrix(float v00, float v01, float v02, float v03, float v10, float v11, float v12, float v13,
...
#pragma region Misc.
static auto FTOC(float a) -> unsigned char {
...
#pragma region Graphics Vertex Definitions
...
struct RageModelVertex { // doesn't have color.  Relies on material color
	...
	/* Zero out by default. */
	RageModelVertex(): p(0, 0, 0), n(0, 0, 0),
	                   t(0, 0) , TextureMatrixScale(1, 1) {}
	RageVector3 p; // position
	RageVector3 n; // normal
	RageVector2 t; // texture coordinates
	int8_t bone{ 0 };
