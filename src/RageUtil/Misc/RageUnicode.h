 ...
 * @return true if a valid encoded string was found. */
bool
utf8_to_wchar(char const* s, size_t iLength, unsigned& start, wchar_t& ch);
 ...
 *
 * @return the appropriate wide character if one exists, or an appropiate
 * default character if one does not. */
wchar_t
utf8_get_char(std::string const& s);
...
/** @brief Remove the starting Byte Order Mark (BOM) form the string if it
 * exists. */
void
...
std::wstring
make_lower(std::wstring const& s);

/** @brief Convert a wide string to its lowercase variant. */
wchar_t
make_lower(wchar_t const& s);

/** @brief Convert a wide string to its uppercase variant. */
std::wstring
make_upper(std::wstring const& s);

/** @brief Convert a wide string to its uppercase variant. */
wchar_t
make_upper(wchar_t const& s);
