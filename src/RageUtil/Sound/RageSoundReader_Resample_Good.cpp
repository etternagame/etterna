 ...
 */
void
ApplyKaiserWindow(float* pBuf, int iLen, float fBeta)
{
...
void
MultiplyVector(float* pStart, float* pEnd, float f)
{
...
void
GenerateSincLowPassFilter(float* pFIR, int iWinSize, float fCutoff)
{
...
struct PolyphaseFilter
{
	struct State
	{
		...
		/* This buffer is duplicated.  If the circular buffer is size L, the
		 * actual buffer is size L*2, and data at buf[N] is also at buf[N+L].
		 * That way, we can access up to buf[N*2-1] without having to wrap. */
	...
	int RunPolyphaseFilter(State& State,
						   const float* pIn,
						   int iSamplesIn,
						   int iDownFactor,
						   float* pOut,
	...
	int NumInputsForOutputSamples(const State& State,
								  int iOut,
								  int iDownFactor) const;
 ...
 */
void
PolyphaseFilter::Generate(const float* pFIR)
{
 ...
 */
int
PolyphaseFilter::RunPolyphaseFilter(State& State,
									const float* pIn,
									int iSamplesIn,
									int iDownFactor,
									float* pOut,
 ...
 */
int
PolyphaseFilter::NumInputsForOutputSamples(const State& State,
										   int iOut,
										   int iDownFactor) const
{
	int iIn = 0;
...
const PolyphaseFilter*
FindNearestPolyphaseFilter(int iUpFactor, float fCutoffFrequency)
{
	/* Find a cached filter with the same iUpFactor and a nearby cutoff
...
{
  public:
	/* Note that going outside of [iMinDownFactor,iMaxDownFactor] while
	 * resampling isn't fatal.  It'll only cause aliasing, by not having a LPF
	 * that's low enough, or cause too much filtering, by not having a LPF
	 * that's high enough. */
	RageSoundResampler_Polyphase(int iUpFactor,
								 int iMinDownFactor,
								 int iMaxDownFactor)
	{
		 ...
		 * when filtering.  This will only cause the low-pass filter to be
		 * rounded; the conversion ratio will always be exact. */
		m_iUpFactor = iUpFactor;
		m_pPolyphase = NULL;
		...
		for (int iDownFactor = iMinDownFactor; iDownFactor <= iMaxDownFactor;
			...
			PolyphaseFilterCache::MakePolyphaseFilter(m_iUpFactor,
													  fCutoffFrequency);
		}
	...
	int Run(const float* pIn,
			int iSamplesIn,
			float* pOut,
			int iSamplesOut,
			int iSampleStride) const
	{
		return m_pPolyphase->RunPolyphaseFilter(*m_pState,
												pIn,
												iSamplesIn,
												m_iDownFactor,
												pOut,
...
int
RageSoundReader_Resample_Good::GetNextSourceFrame() const
{
...
float
RageSoundReader_Resample_Good::GetStreamToSourceRatio() const
{
...
void
RageSoundReader_Resample_Good::Reset()
{
...
void
RageSoundReader_Resample_Good::GetFactors(int& iDownFactor,
										  int& iUpFactor) const
{
...
void
RageSoundReader_Resample_Good::ReopenResampler()
{
 ...
 */
int
RageSoundReader_Resample_Good::SetPosition(int iFrame)
{
	Reset();
	iFrame = (int)SCALE(iFrame,
						0,
						(int64_t)m_iSampleRate,
						0,
...
int
RageSoundReader_Resample_Good::Read(float* pBuf, int iFrames)
{
	...
	{
		int iFramesNeeded =
		  m_apResamplers[0]->NumInputsForOutputSamples(iFrames);
		float* pTmpBuf =
		  (float*)alloca(iFramesNeeded * sizeof(float) * iChannels);
		int iFramesIn = m_pSource->Read(pTmpBuf, iFramesNeeded);
		if (iFramesIn < 0)
			return iFramesIn;

		for (int iChannel = 0; iChannel < iChannels; ++iChannel) {
			int iGotFrames = m_apResamplers[iChannel]->Run(pTmpBuf + iChannel,
														   iFramesIn,
														   pBuf + iChannel,
														   iFrames,
														   iChannels);
 ...
 */
void
RageSoundReader_Resample_Good::SetRate(float fRatio)
{
	ASSERT(fRatio > 0);
...
float
RageSoundReader_Resample_Good::GetRate() const
{
	if (m_fRate == -1)
		return 1.0f;
	else
