...
void
test_suspend_threadid(uint64_t ThreadId)
{
...
void
TestBacktraceThreadLoop() __attribute__((__noinline__));
void
...
int
TestBacktraceThread(void* p) __attribute__((__noinline__));
int
TestBacktraceThread(void* p)
{
	TestBacktraceThreadLoop();
	return 0;
}
...
bool
test_thread_backtrace(int ThreadId, const void* expect1, const void* expect2)
{
	...
	for (int i = 0; BacktracePointers[i]; ++i) {
		...
		//		printf("want %p, %p: %p, %p\n", expect1, expect2,
		// bn.Address-bn.Offset, BacktracePointers[i] ); 		printf("
		//%s\n", bn.Format().c_str() );

		/* bn.Address is the current address; Offset is the distance to the
		 * beginning of the symbol, so bn.Address-bn.Offset is the actual
		 * symbol. */
...
test_backtracing_secondary_thread()
{
	...
	if (!test_thread_backtrace(g_ThreadId,
							   (void*)TestBacktraceThread,
...
int
TestBacktraceMainThread(void* p)
{
	printf("Test thread started\n");
