...
void
ReadData(RageSoundReader* pReader,
		 int iFrame,  /* start */
		 float* pBuf, /* out */
		 int iFrames)
{
...
void
find(const char* haystack, int hs, const char* needle, int ns)
{
...
void
dump_bin(const char* fn, const char* buf, int size)
{
...
void
dump(const char* fn, const int16_t* buf, int samples)
{
	FILE* f = fopen(fn, "w+");
	ASSERT(f);
...
void
dump(const int16_t* buf, int samples)
{
	for (int i = 0; i < samples; ++i)
...
void
dump(const float* buf, int samples)
{
	for (int i = 0; i < samples; ++i)
...
bool
compare(const float* m1, const int16_t* m2, int iSamples)
{
...
void
compare_buffers(const int16_t* expect,
				const int16_t* got,
				int frames,
				...
				int channels)
{
	 ...
	 *
	 * 2: either the source or dest data starts out around 0 and converges
	 * quickly on the other; this happens with resamplers after a seek, since
	 * they're missing data before the seeked position (could be fixed)
	 *
	 * When seeking, resamplers should ideally seek slightly before the
	 * requested position, read the extra data, feed it to the resampling
	 * engine, and discard the extra data. Otherwise, the resampler is starting
	 * with no data, and will give some low samples. (The amount of incorrect
	 * data depends on the window size of the resampler.) Detect if this
	 * happens: determine if the data starts very inaccurately and quickly
	 * converges.
	 *
...
bool
test_read(RageSoundReader* snd, float* expected_data, int frames)
{
...
bool
must_be_eof(RageSoundReader* snd)
{
	float buf[16];
...
RageSoundReader*
ApplyFilters(RageSoundReader* s, int filters)
{
...
bool
CheckSetPositionAccurate(RageSoundReader* snd)
{
...
int
FramesOfSilence(const float* data, int frames, int iChannels)
{
	int SilentFrames = 0;
 ...
 * find the closest. */
void*
xmemsearch(const float* haystack,
		   size_t iHaystackSamples,
		   const int16_t* needle,
		   size_t iNeedleSamples,
		   int expect)
{
	if (!iNeedleSamples)
	...
	while (1) {
		...
		if (hay_early >= haystack &&
			hay_early + iNeedleSamples < pHaystackEnd) {
...
bool
RunTests(RageSoundReader* snd, const TestFile& tf)
{
	const char* fn = tf.fn;
	{
...

	{
		...
		if (SilentFrames != tf.SilentFrames) {
			LOG->Trace("Expected %i silence, got %i (%i too high)",
					   tf.SilentFrames,
					   SilentFrames,
					   SilentFrames - tf.SilentFrames);
			bFailed = true;
		}
		...
		const int LaterOffsetSamples =
		  LaterOffsetFrames * snd->GetNumChannels();
		...
		if (!Identical) {
			...
			float* p = (float*)xmemsearch(sdata,
										  one_second_bytes,
										  tf.later,
										  sizeof(tf.later),
			...
			if (p) {
				...
				LOG->Trace("Found half second data at frame %i (wanted %i), "
						   "ahead by %i samples",
						   FramesOff,
						   LaterOffsetFrames,
						   FramesOff - LaterOffsetFrames);
			}
			//			else
	...
	/* Make sure we're getting non-null data. */
	{
		...
		if (bAllNull || bAll42) {
			LOG->Warn(
			  "'%s': sanity check failed (%i %i)", fn, bAllNull, bAll42);
			return false;
		}
	}
	...
	for (int i = 0; i < 2; ++i) {
		...
		if (iRet != 1) {
			LOG->Warn("Fail: SetPosition(0) (%s) returned %i, expected 1",
					  szMode,
					  iRet);
			return false;
		}
		...
		if (ret2 != 0) {
			LOG->Warn(
			  "Fail: SetPosition(1000000) (%s) returned %i instead of 0",
			  szMode,
			  ret2);
			return false;
		}
	...
	snd->SetPosition(iFrame);
	if (!test_read(snd,
				   sdata + one_second_samples * 1 / 1000,
	...
	snd->SetPosition(iFrame);
	if (!test_read(snd,
				   sdata + one_second_samples * 500 / 1000,
...
bool
test_file(const TestFile& tf, int filters)
{
	const char* fn = tf.fn;
...
main(int argc, char* argv[])
{
	...
	*/
		...
		/* The following all contain the same data; they simply have
		   different tags and headers. */
