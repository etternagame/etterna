...
void
CreateBinaryTestFile(RageFile& f, int size)
{
	char c = 0;
	while (size--) {
...
void
CreateBinaryTestFile(RString fn, int size)
{
	RageFile f;
	...
	if (!f.Open(fn)) {
		LOG->Warn("CreateBinaryTestFile: error reopening %s: %s",
				  fn.c_str(),
				  f.GetError().c_str());
		return;
	}
	...
	if (test != size)
		LOG->Warn(
		  "CreateBinaryTestFile: f.GetFileSize ret %i, exp %i", test, size);
}

bool
CheckData(const RString& buf, int offset)
{
	char c = offset % 256;
			   ...
			   int TestDataPos)
{
	...
	if (test.Tell() != ExpectedFilePos + ret) {
		LOG->Warn(
		  "Simple Tell() failed (got %i, expected %i)", test.Tell(), ret);
		return -1;
	}
	return 1;
}
...
RString
MakeTextTestLine(int line, int size)
{
...
bool
CheckTextData(const RString& buf, int LineSize, int size)
{
	return buf == MakeTextTestLine(LineSize, size);
}

void
TestText(int LineSize, bool DOS)
{
...
TestText()
{
	/* Test with these line sizes.  The internal buffer size is 256, so test
	 * line sizes around there carefully. */
...
TestSeek(bool relative)
{
	...
	for (int pass = 0; pass < 2; ++pass) {
		...
		if (ret != int(TestLine.size() + junk.size())) {
			LOG->Warn(
			  "Seek(%i) failed: got %i", TestLine.size() + junk.size(), ret);
			return;
		}
			 ...
			 int iBlockSize)
{
	...
	while (iGot < iSize) {
		...
		if (iRet == 0) {
			LOG->Warn(
			  "Premature EOF (iSize %i, iBlockSize %i)", iSize, iBlockSize);
			return;
		}
...
main(int argc, char* argv[])
{
	...
	if (CreateTestFiles)
		...
		if (!test.Open("test.binary")) {
			LOG->Warn("Open \"test.binary\" failed: %s",
					  test.GetError().c_str());
			exit(1);
		}
		...
		if (test.Tell() != 0) {
			LOG->Warn("Simple Tell() after reopen failed (got %i, expected 0)",
					  test.Tell());
			exit(1);
		}
