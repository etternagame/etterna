...
#if !defined(                                                                  \
  AFX_ZIPEXCEPTION_H__E3546921_D728_11D3_B7C7_E77339672847__INCLUDED_)
#define AFX_ZIPEXCEPTION_H__E3546921_D728_11D3_B7C7_E77339672847__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#pragma warning(push)
#pragma warning(disable : 4702) // disable "Unreachable code" warning in Throw
								// function in the Release mode
#endif							// _MSC_VER > 1000
...
*/
	...
	static void Throw(int iZipError = CZipException::generic,
	...
	/**
	Convert a zlib library error code to a \link #ZipErrors CZipException error
	code \endlink \param	iZlibError zlib library error code \return	\link
	#ZipErrors CZipException error code \endlink
*/
	...
	(this function is provided only for compatibility with MFC \c
	...
	*/
	...
	{
		noError,	   ///< no error
					   // 			 1 - 42 reserved for errno (from STL) values -
					   // used only in non-MFC versions 			 43 - 99 reserved
		...
		aborted, ///< callback functor's method Callback returned \c false while
				 ///< disk change in the disk-spanning archive
		abortedAction, ///< callback functor's method Callback returned \c false
					   ///< in CZipArchive class members: AddNewFile,
					   ///< ExtractFile, TestFile, DeleteFile or DeleteFiles
		abortedSafely, ///< the same as above, you may be sure that the
					   ///< operation was successfully completed before or it
					   ///< didn't cause any damage in the archive (break when
					   ///< counting before deleting files; see
		...
		notRenamed,   ///< error while renaming a file (under Windows call
					  ///< GetLastError() to find out more)
		platfNotSupp, ///< the platform that the zip file is being created for
					  ///< is not supported
		cdirNotFound, ///< the central directory was not found in the archive
					  ///< (it is thrown also when the last disk of multi-disk
					  ///< archive is not in the drive when opening the archive)
