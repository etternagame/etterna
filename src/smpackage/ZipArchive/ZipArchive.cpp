...
#define ZIP_COMPR_REPL_SIGN                                                    \
	0x0100 // first 8 bits should be 00 (reserved for compression level), next 8
		   // should be different from ff (to distinguish from -1)

const TCHAR CZipArchive::m_gszCopyright[] = {
	_T("ZipArchive library Copyright 2000 - 2003 Tadeusz Dracz")
};

void
CZipAddNewFileInfo::Defaults()
{
...
CZipArchive::~CZipArchive()
{
	// 	Close(); // cannot be here: if an exception is thrown strange things can
	// happen
	EmptyPtrList();
}

void
CZipArchive::Open(LPCTSTR szPathName, int iMode, int iVolumeSize)
{
	if (!IsClosed()) {
...
void
CZipArchive::Open(CZipMemFile& mf, int iMode)
{
	if (!IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive already opened.\n"), __FILE__, __LINE__);
		return;
...
void
CZipArchive::OpenInternal(int iMode)
{
...
bool
CZipArchive::IsClosed(bool bArchive) const
{
	return bArchive ? (m_storage.GetCurrentDisk() == -1)
					: (!m_storage.m_pFile || m_storage.m_pFile->IsClosed());
}

void
CZipArchive::ThrowError(int err, bool bZlib)
{
	if (bZlib)
		err = CZipException::ZlibErrToZip(err);
	CZipException::Throw(
	  err, IsClosed() ? _T("") : (LPCTSTR)m_storage.m_pFile->GetFilePath());
}

bool
CZipArchive::GetFileInfo(CZipFileHeader& fhInfo, WORD uIndex) const
{
	if (IsClosed()) {
...
int
CZipArchive::FindFile(LPCTSTR lpszFileName,
					  int iCaseSensitive,
					  bool bFileNameOnly)
{
	if (IsClosed()) {
	...
	return m_centralDir.FindFile(
	  lpszFileName, bCS, bSporadically, bFileNameOnly);
}

bool
CZipArchive::OpenFile(WORD uIndex)
{
	if (!m_centralDir.IsValidIndex(uIndex)) {
		ASSERT(FALSE);
		return false;
	...
	if (CurrentFile()->IsEncrypted()) {
		...
		TRACE(_T("%s(%i) : Password set for a not encrypted file. Ignoring ")
			  _T("password.\n"),
			  __FILE__,
			  __LINE__);
	}
...
int
CZipArchive::GetLocalExtraField(char* pBuf, int iSize) const
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return -1;
	...
	if (m_iFileOpened != extract) {
		TRACE(
		  _T("%s(%i) : A file must be opened to get the local extra field.\n"),
		  __FILE__,
		  __LINE__);
		return -1;
	}
...
void*
CZipArchive::_zliballoc(void* opaque, UINT items, UINT size)
{
...
void
CZipArchive::_zlibfree(void* opaque, void* address)
{
	if (opaque) {
		CZipPtrList<void*>* list = (CZipPtrList<void*>*)opaque;
...
void
CZipArchive::CheckForError(int iErr)
{
...
DWORD
CZipArchive::ReadFile(void* pBuf, DWORD iSize)
{
	if (m_iFileOpened != extract) {
		TRACE(
		  _T("%s(%i) : Current file must be opened.\n"), __FILE__, __LINE__);
		return 0;
	}
	...
	m_info.m_stream.avail_out =
	  iSize > m_info.m_uUncomprLeft ? m_info.m_uUncomprLeft : iSize;
	...
	while (m_info.m_stream.avail_out > 0 ||
		...
		if (CurrentFile()->m_uMethod == 0) {
			...
			m_info.m_uCrc32 =
			  crc32(m_info.m_uCrc32, m_info.m_stream.next_out, uToCopy);
...
void
CZipArchive::Close(int iAfterException, bool bUpdateTimeStamp)
{
	// if after an exception - the archive may be closed, but the file may be
	// opened
	if (IsClosed() && (!iAfterException || IsClosed(false))) {
		TRACE(
		  _T("%s(%i) : ZipArchive is already closed.\n"), __FILE__, __LINE__);
		return;
	}
	...
	if (iAfterException != afAfterException &&
		!IsClosed(false))			  // in disk spanning when user aborts
...
void
CZipArchive::WriteCentralDirectory(bool bFlush)
{
...
void
CZipArchive::SetCallback(CZipActionCallback* pCallback, int iWhich)
{
	CallbackType cbs[] = { cbAdd,	 cbAddTmp,	cbAddStore,
						   cbExtract, cbDeleteCnt, cbDelete,
						   cbTest,	cbSave,	  cbGetFromArchive,
						   cbRename,  cbReplace };
...
void
CZipArchive::SetAdvanced(int iWriteBuffer,
						 int iGeneralBuffer,
						 int iSearchBuffer)
{
	if (!IsClosed()) {
...
int
CZipArchive::CloseFile(CZipFile& file)
{
...
int
CZipArchive::CloseFile(LPCTSTR lpszFilePath, bool bAfterException)
{
	if (m_iFileOpened != extract) {
	...
	if (!bAfterException) {
		...
		if (lpszFilePath) {
			...
			if (!ZipPlatform::SetFileModTime(lpszFilePath,
											 CurrentFile()->GetTime()) ||
				!ZipPlatform::SetFileAttr(lpszFilePath,
										  CurrentFile()->GetSystemAttr()))
				iRet = -2;
		}
	}
...
bool
CZipArchive::OpenNewFile(CZipFileHeader& header,
						 int iLevel,
						 ...
						 DWORD uInternal)
{
	...
	if (m_storage.IsSpanMode() == -1) {
		TRACE(_T("%s(%i) : You cannot add files to the existing disk spannig ")
			  _T("archive.\n"),
			  __FILE__,
			  __LINE__);
		return false;
	...
	if (lpszFilePath) {
		...
		if (!ZipPlatform::GetFileAttr(lpszFilePath, uAttr))
			// do not continue - if the file was a directory then not
			// recognizing it will cause serious errors (need uAttr to recognize
			// it)
			return false;
	...
	if (bIsDirectory) {
		...
		if (!iNameLen ||
			!CZipPathComponent::IsSeparator(szFileName[iNameLen - 1])) {
	...
	// the find array keeps all the files already converted to the current
	// system standards we do not perform Oem->Ansi here, because who would pass
	// oem values here?
	//
	...
	if (bReplace) {
		...
		if (header.IsDataDescr())
			uInternal +=
			  ZIPARCHIVE_DATADESCRIPTOR_LEN + 4; // CZipCentralDir::CloseNewFile
	}
	m_centralDir.AddNewFile(header, iReplaceIndex);
	if (bReplace)
		MakeSpaceForReplace(iReplaceIndex, uInternal, szFileName);

	// this ensures the conversion will take place anyway (must take because we
	// are going 	to write the local header in a moment
	...
	if (CurrentFile()->m_uMethod == Z_DEFLATED) {
		...
		int err = deflateInit2(&m_info.m_stream,
							   iLevel,
							   Z_DEFLATED,
							   -MAX_WBITS,
							   DEF_MEM_LEVEL,
...
bool
CZipArchive::ExtractFile(WORD uIndex,
						 LPCTSTR lpszPath,
						 bool bFullPath,
						 LPCTSTR lpszNewName,
						 DWORD nBufSize)
{
	...
	GetFileInfo(header,
				uIndex); // to ensure that slash and oem conversions take place
	CZipString szFileNameInZip = (LPCTSTR)header.GetFileName();
	CZipString szFile = PredictExtractedFileName(
	  szFileNameInZip, lpszPath, bFullPath, lpszNewName);
	...
	if (header.IsDirectory()) {
		...
		CZipFile f(szFile,
				   CZipFile::modeWrite | CZipFile::modeCreate |
					 CZipFile::shareDenyWrite);
		DWORD iRead;
		...
		do {
			...
			if (iRead) {
				...
				if (pCallback)
					if (!(*pCallback)(iRead)) {
						if (iRead == buf.GetSize() &&
							ReadFile(buf, 1) !=
							  0) // test one byte if there is something left
							iAborted = CZipException::abortedAction;
						else
							iAborted =
							  CZipException::abortedSafely; // we did it!
						break;
					}
			}
		...
		if (!bRet && iAborted == CZipException::abortedSafely)
			iAborted = CZipException::abortedAction; // sorry, finished, but not
													 // successfull
		...
		if (iAborted)
			CZipException::Throw(
			  iAborted,
			  szFile); // throw to distuingiush from other return codes
		return bRet;
	}
}

bool
CZipArchive::ExtractFile(WORD uIndex, CZipMemFile& mf, DWORD nBufSize)
{
	if (!nBufSize)
		return false;
	...
	if (!bRet && iAborted == CZipException::abortedSafely)
		iAborted =
		  CZipException::abortedAction; // sorry, finished, but not successfull
	...
	if (iAborted)
		CZipException::Throw(
		  iAborted); // throw to distuingiush from other return codes
	return bRet;
}

void
CZipArchive::SetExtraField(const char* pBuf, WORD iSize)
{
	if (m_iFileOpened != compress) {
...
bool
CZipArchive::WriteNewFile(const void* pBuf, DWORD iSize)
{
	if (m_iFileOpened != compress) {
		TRACE(_T("%s(%i) : A new file must be opened.\n"), __FILE__, __LINE__);
		return false;
	...
	CurrentFile()->m_uCrc32 =
	  crc32(CurrentFile()->m_uCrc32, (Bytef*)pBuf, iSize);
...
bool
CZipArchive::CloseNewFile(bool bAfterException)
{
	if (m_iFileOpened != compress) {
		TRACE(_T("%s(%i) : A new file must be opened.\n"), __FILE__, __LINE__);
		return false;
		...
		if (CurrentFile()->m_uMethod == Z_DEFLATED)
			while (err == Z_OK) {
				if (m_info.m_stream.avail_out == 0) {
					CryptEncodeBuffer();
					m_storage.Write(
					  m_info.m_pBuffer, m_info.m_uComprLeft, false);
					m_info.m_uComprLeft = 0;
					m_info.m_stream.avail_out = m_info.m_pBuffer.GetSize();
					m_info.m_stream.next_out = (Bytef*)(char*)m_info.m_pBuffer;
				}
...
void
CZipArchive::DeleteFile(WORD uIndex)
{
...
void
CZipArchive::GetIndexes(const CZipStringArray& aNames, CZipWordArray& aIndexes)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return;
...
void
CZipArchive::DeleteFiles(const CZipStringArray& aNames)
{
	CZipWordArray indexes;
...
void
CZipArchive::DeleteFiles(CZipWordArray& aIndexes)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return;
	...
	if (m_storage.IsSpanMode()) {
		TRACE(_T("%s(%i) : You cannot delete files from the disk spannig ")
			  _T("archive.\n"),
			  __FILE__,
			  __LINE__);
		return;
	...
	if (m_iFileOpened) {
		TRACE(
		  _T("%s(%i) : You cannot delete files if there is a file opened.\n"),
		  __FILE__,
		  __LINE__);
		return;
	}
	...
	if (!uSize) {
		TRACE(
		  _T("%s(%i) : The indekses array is empty.\n"), __FILE__, __LINE__);
		return;
	}
	...
	DWORD uTotalToMoveBytes = 0, uLastOffset = m_storage.m_pFile->GetLength() -
											   m_centralDir.GetBytesBefore();
	...
	for (i = 0; i < uSize; i++) {
		...
		if (di.m_bDelete) {
			...
			if (uOffsetStart) {
				...
				uOffsetStart = 0; // never be at the beginning, because the
								  // first file is always to be deleted
			}
			if (i == uSize - 1)
				uTemp = (m_storage.m_pFile->GetLength() -
						 m_centralDir.GetBytesBefore()) -
						uTemp;
			else
...
bool
CZipArchive::AddNewFile(LPCTSTR lpszFilePath,
						int iComprLevel,
						bool bFullPath,
						int iSmartLevel,
...
bool
CZipArchive::AddNewFile(CZipMemFile& mf,
						LPCTSTR lpszFileNameInZip,
						int iComprLevel,
						int iSmartLevel,
						unsigned long nBufSize)
...
bool
CZipArchive::AddNewFile(CZipAddNewFileInfo& info)
{
	...
	if (ZipPlatform::GetSystemID() != ZipCompatibility::zcUnix)
		uAttr |= ZipCompatibility::ConvertToSystem(
		  uAttr,
		  ZipPlatform::GetSystemID(),
		  ZipCompatibility::zcUnix); // make it readable under Unix as well,
									 // since it stores its attributes in
									 // HIWORD(uAttr)
	...
	if (header.IsDirectory()) // will never be when m_pFile is not NULL, so we
							  // don't check it
	{
		...
		bRet = OpenNewFile(
		  header,
		  bReplace ? (info.m_iReplaceIndex << 16) | ZIP_COMPR_REPL_SIGN : 0);
	...
	bool bCheckForZeroSized =
	  (info.m_iSmartLevel & zipsmCPFile0) && !GetPassword().IsEmpty();
	bool bCheckForSmallFiles =
	  (info.m_iSmartLevel & zipsmNotCompSmall) && bIsCompression;
	...
	if (bCheckForSmallFiles || bCheckForZeroSized || bNeedTempArchive) {
		...
		if (info.m_pFile)
			...
			if (!ZipPlatform::GetFileSize(info.m_szFilePath, iFileSize) && bEff)
				bEff =
				  false; // the file size is needed only when eff. in span mode
		}
	...
	if (bNeedTempArchive &&
		...
		CZipArchive zip;
		try {
			// compress first to a temporary file, if ok - copy the data, if not
			// - add storing
			...
			m_info.Init();
			throw GetFromArchive(
			  zip, 0, info.m_iReplaceIndex, true, GetCallback(cbAddTmp));
		} catch (bool bRet) {

			zip.Close(
			  !bRet); // that doesn't really matter how it will be closed
			if (pmf)
				delete pmf;
			if (!bEffInMem)
	...
	if (info.m_pFile)
		...
		// : If you specify the GENERIC_READ and GENERIC_WRITE access modes
		// along with the FILE_SHARE_READ and FILE_SHARE_WRITE sharing modes in
		// your first call to CreateFile. If you specify the GENERIC_READ and
		// GENERIC_WRITE access modes and the FILE_SHARE_READ sharing mode only
		// in your second call to CreateFile, the function will fail with a
		// sharing violation because the read-only sharing mode specified in the
		// second call conflicts with the read/write access that has been
		// granted in the first call.
		if (!f.Open(info.m_szFilePath,
					CZipFile::modeRead | CZipFile::shareDenyNone,
					false)) {
			if (pCallback)
				pCallback->CallbackEnd();
			return false;
	...
	if (bReplace) {
		...
		bRet = OpenNewFile(header,
						   (info.m_iReplaceIndex << 16) | ZIP_COMPR_REPL_SIGN,
						   NULL,
						   iFileSize);
	} else
	...
	do {
		...
		if (iRead) {
			...
			if (pCallback)
				if (!(*pCallback)(iRead)) {
					// todo: we could remove here the bytes of the file
					// partially added if not disk-spanning
					if (iRead == buf.GetSize() &&
						pf->Read(buf, 1) !=
						  0) // test one byte if there is something left
					{
	...
	if (iAborted)
		CZipException::Throw(
		  iAborted); // throw to distuinguish from other return codes
...
bool
CZipArchive::RemoveLast(bool bRemoveAnyway)
{
...
CZipString
CZipArchive::GetArchivePath() const
{
	if (IsClosed(false)) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return _T("");
...
CZipString
CZipArchive::GetGlobalComment() const
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return _T("");
	...
	CZipString temp;
	return SingleToWide(m_centralDir.m_pszComment, temp) != -1 ? (LPCTSTR)temp
															   : _T("");
}

bool
CZipArchive::SetGlobalComment(LPCTSTR lpszComment)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return false;
	...
	if (m_storage.IsSpanMode() == -1) {
		TRACE(_T("%s(%i) : You cannot modify the global comment of the ")
			  _T("existing disk spanning archive.\n"),
			  __FILE__,
			  __LINE__);
		return false;
	}
...
int
CZipArchive::GetCurrentDisk() const
{
	return m_storage.GetCurrentDisk() + 1;
}

bool
CZipArchive::SetFileComment(WORD uIndex, LPCTSTR lpszComment)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return false;
	...
	if (m_storage.IsSpanMode() == -1) {
		TRACE(_T("%s(%i) : You cannot modify the file comment in the existing ")
			  _T("disk spanning archive.\n"),
			  __FILE__,
			  __LINE__);
		return false;
	}
...
void
CZipArchive::CryptUpdateKeys(char c)
{
...
CZipArchive::CryptCheck()
{
	...
	return CurrentFile()->IsDataDescr()
			 ? (BYTE(CurrentFile()->m_uModTime >> 8) == b)
			 : (BYTE(CurrentFile()->m_uCrc32 >> 24) == b);
}

char
CZipArchive::CryptDecryptByte()
{
...
void
CZipArchive::CryptDecode(char& c)
{
	c ^= CryptDecryptByte();
	CryptUpdateKeys(c);
}
...
bool
CZipArchive::SetPassword(LPCTSTR lpszPassword)
{
	if (m_iFileOpened != nothing) {
		TRACE(_T("%s(%i) : You cannot change the password when the file is ")
			  _T("opened.\n"),
			  __FILE__,
			  __LINE__);
		return false; // it's important not to change the password when the file
					  // inside archive is opened
	}
	if (IsClosed()) {
		TRACE(_T("%s(%i) : Setting the password for a closed archive has no ")
			  _T("effect.\n"),
			  __FILE__,
			  __LINE__);
	}
	if (lpszPassword) {
		...
		for (size_t i = 0; (int)i < iLen; i++)
			if (m_pszPassword[i] <= 0) {
				m_pszPassword.Release();
				TRACE(_T("%s(%i) : The password contains forbidden ")
					  _T("characters. Password cleared.\n"),
					  __FILE__,
					  __LINE__);
				return false;
			}
...
DWORD
CZipArchive::CryptCRC32(DWORD l, char c)
{
...
void
CZipArchive::CryptCryptHeader(long iCrc, CZipAutoBuffer& buf)
{
	CryptInitKeys();
...
void
CZipArchive::CryptEncode(char& c)
{
...
bool
CZipArchive::TestFile(WORD uIndex, DWORD uBufSize)
{
	if (m_storage.IsSpanMode() == 1) {
		TRACE(
		  _T("%s(%i) : You cannot test the spanning archive in creation.\n"),
		  __FILE__,
		  __LINE__);
		return false;
	}
	...
	if (pHeader->IsDirectory()) {
		...
		if ((iSize != 0 || iSize != pHeader->m_uUncomprSize)
			...
			&& !(pHeader->IsEncrypted() && iSize == 12 &&
				 !pHeader->m_uUncomprSize))
		...
		try {
			...
			bool bRet = CloseFile() != -1;
			if (!bRet && iAborted == CZipException::abortedSafely)
				iAborted = CZipException::abortedAction; // sorry, finished, but
														 // not successfull
...
int
CZipArchive::WideToSingle(LPCTSTR lpWide, CZipAutoBuffer& szSingle)
{
#ifdef _UNICODE
...
int
CZipArchive::SingleToWide(const CZipAutoBuffer& szSingle, CZipString& szWide)
{
...
void
CZipArchive::CryptDecodeBuffer(DWORD uCount)
{
	if (CurrentFile()->IsEncrypted())
...
void
CZipArchive::EmptyPtrList()
{
	if (m_list.GetCount()) {
		// if some memory hasn't been freed due to an error in zlib, so free it
		// now
...
void
CZipArchive::SetFileHeaderAttr(CZipFileHeader& header, DWORD uAttr)
{
	header.SetSystemCompatibility(m_iArchiveSystCompatib);
...
void
CZipArchive::EnableFindFast(bool bEnable)
{
	if (IsClosed()) {
...
bool
CZipArchive::SetSystemCompatibility(int iSystemComp)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : Set it after opening the archive.\n"),
			  __FILE__,
...
void
CZipArchive::SetRootPath(LPCTSTR szPath)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : Set it after opening the archive.\n"),
			  __FILE__,
...
CZipString
CZipArchive::TrimRootPath(CZipPathComponent& zpc) const
{
	if (m_szRootPath.IsEmpty())
		return zpc.GetFileName();
	CZipString szPath = zpc.GetFullPath();
	return RemovePathBeginning(m_szRootPath, szPath, m_pZipCompare)
			 ? szPath
			 : zpc.GetFileName();
}

bool
CZipArchive::RemovePathBeginning(LPCTSTR lpszBeginning,
								 CZipString& szPath,
...
void
CZipArchive::SetTempPath(LPCTSTR lpszPath, bool bForce)
{
...
CZipString
CZipArchive::PredictFileNameInZip(LPCTSTR lpszFilePath,
								  bool bFullPath,
								  int iWhat,
								  bool bExactly) const
{
	...
	if (bExactly) {
		...
		fh.SetSystemCompatibility(
		  -1); // non existing system to prevent ansi oem conversion
		ZipCompatibility::FileNameUpdate(fh,
										 true); // update only path separators
	}
...
CZipString
CZipArchive::PredictExtractedFileName(LPCTSTR lpszFileNameInZip,
									  LPCTSTR lpszPath,
									  bool bFullPath,
...
void
CZipArchive::SetAutoFlush(bool bAutoFlush)
{
	if (IsClosed()) {
	...
	if (m_storage.IsSpanMode() != 0) {
		TRACE(
		  _T("%s(%i) : Cannot set auto-flush for the disk spanning archive.\n"),
		  __FILE__,
		  __LINE__);
		return;
	}
...
CZipArchive::Flush()
{
	...
	m_storage.FlushFile();
	if (m_storage.IsSpanMode() >
		0) // try to finalize disk-spanning archive without closing it
		m_storage.FinalizeSpan();
}

void
CZipArchive::GetCentralDirInfo(CZipCentralDir::Info& info) const
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive not yet opened.\n"), __FILE__, __LINE__);
		return;
...
bool
CZipArchive::CWildcard::IsPatternValid(LPCTSTR lpszPattern, int* iErrorType)
{
	try {
...
bool
CZipArchive::CWildcard::IsPattern(LPCTSTR lpszPattern)
{
	while (*lpszPattern) {
		switch (*lpszPattern++) {
			case _T('?'):
...
bool
CZipArchive::CWildcard::IsMatch(LPCTSTR lpszText, int* iRetCode)
{
	CZipString sz;
	if (!m_bCaseSensitive) {
		sz = lpszText;
...
int
CZipArchive::CWildcard::MatchAfterStar(LPCTSTR p, LPCTSTR t)
{
	...
	} while (iMatch != matchValid && iMatch != matchAbort &&
			 iMatch != matchPattern);
...
int
CZipArchive::CWildcard::Match(LPCTSTR lpszPattern, LPCTSTR lpszText)
{
			...
			case _T('['): {
				...
				while (bLoop) {
					...
					if (range_start < range_end) {
						if (*lpszText >= range_start &&
							*lpszText <= range_end) {
							bMemberMatch = true;
							bLoop = false;
						}
					} else {
						if (*lpszText >= range_end &&
							*lpszText <= range_start) {
							bMemberMatch = true;
							bLoop = false;
						}
					}
...
void
CZipArchive::FindMatches(LPCTSTR lpszPattern,
						 CZipWordArray& ar,
...
int
CZipArchive::WillBeDuplicated(LPCTSTR lpszFilePath,
							  bool bFullPath,
							  bool bFileNameOnly,
							  int iWhat)
{
	CZipString szFile;
	// we predict with bExactly set to false, because FindFile converts all
	// filanames anyway
	if (bFileNameOnly) {
...
// it'll get up to the next file or to the end of file (bad if zip corrupted or
// not-ordered by offsett or redundant bytes added)
bool
CZipArchive::GetFromArchive(CZipArchive& zip,
							WORD uIndex,
							int iReplaceIndex,
							bool bKeepSystComp,
							CZipActionCallback* pCallback)
{
	...
	if (m_iFileOpened || zip.m_iFileOpened) {
		TRACE(_T("%s(%i) : You cannot get files from another archive if there ")
			  _T("is a file opened.\n"),
			  __FILE__,
			  __LINE__);
		return false;
	...
	if (zip.m_storage.IsSpanMode()) {
		TRACE(
		  _T("%s(%i) : You cannot get files from the disk spannig archive.\n"),
		  __FILE__,
		  __LINE__);
		return false;
	}
	...
	DWORD uStartOffset =
	  zip.m_centralDir.GetBytesBefore() + fh.m_uOffset + fh.GetSize(true);
	DWORD uTotalToMove = uEndOffset - uStartOffset, uTotalMoved = 0;

	DWORD uPredictedSize =
	  fh.m_uComprSize + (fh.IsDataDescr() ? ZIPARCHIVE_DATADESCRIPTOR_LEN : 0);
	...
	bool bConvertSystem =
	  !bKeepSystComp && fh.GetSystemCompatibility() != m_iArchiveSystCompatib;

	// GetFileInfo always converts the filename regardless of
	// zip.m_centralDir.m_bConvertAfterOpen value
	...
	// if the same callback is applied to cbReplace, then the previous
	// information about the type will be lost
	CZipFileHeader* pHeader =
	  m_centralDir.AddNewFile(fh, iReplaceIndex); // must be converted when
												  // adding because of
												  // InsertFastElement
	if (bReplace)
		MakeSpaceForReplace(
		  iReplaceIndex, uTotalToMove + fh.GetSize(true), szFileNameConverted);
	...
	DWORD uPack = uTotalToMove > m_info.m_pBuffer.GetSize()
					? m_info.m_pBuffer.GetSize()
					: uTotalToMove;
	...
	// copying from non-span to span or from span without data description to
	// span so add the data descriptor
...
bool
CZipArchive::RenameFile(WORD uIndex, LPCTSTR lpszNewName)
{
	if (IsClosed()) {
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"), __FILE__, __LINE__);
		return false;
	...
	if (m_storage.IsSpanMode()) {
		TRACE(
		  _T("%s(%i) : You cannot rename files in the disk spannig archive.\n"),
		  __FILE__,
		  __LINE__);
		return false;
	}
	...
	if (m_iFileOpened) {
		TRACE(
		  _T("%s(%i) : You cannot rename a file if there is a file opened.\n"),
		  __FILE__,
		  __LINE__);
		return false;
	}
	...
	ZipCompatibility::FileNameUpdate(
	  fh, false); // in case the conversion changes the filename size
	...
	if (iDelta != 0) {
		...
		if (bForward)
			m_storage.m_pFile->SetLength(uFileLen +
										 iDelta); // ensure the seek is correct

		MovePackedFiles(
		  uStartOffset, uEndOffset, abs(iDelta), pCallback, bForward);
		if (pCallback)
			pCallback->CallbackEnd();

		if (!bForward)
			m_storage.m_pFile->SetLength(uFileLen +
										 iDelta); // delta < 0; shrink the file
	...
	m_storage.m_pFile->Seek(m_centralDir.GetBytesBefore() + fh.m_uOffset + 30 +
							  iOffset,
							CZipAbstractFile::begin);
...
bool
CZipArchive::UpdateReplaceIndex(int& iReplaceIndex, LPCTSTR lpszNewFileName)
{
	if (iReplaceIndex == -2)
	...
	if (GetSpanMode() != 0) {
		TRACE(
		  _T("%s(%i) : You cannot replace files in a disk-spanning archive.\n"),
		  __FILE__,
		  __LINE__);
		return false;
	}
...
void
CZipArchive::MakeSpaceForReplace(int iReplaceIndex,
								 DWORD uTotal,
								 LPCTSTR lpszFileName)
{
	...
	DWORD uReplaceStart =
	  m_storage.m_pFile->GetPosition() - m_centralDir.GetBytesBefore();
	...
	if (iDelta != 0) {
		...
		DWORD uUpperLimit =
		  uFileLen -
		  m_centralDir.GetBytesBefore(); // will be added in MovePackedFiles
		if (pCallback) {
		...
		MovePackedFiles(
		  uReplaceEnd, uUpperLimit, abs(iDelta), pCallback, bForward);
...
void
CZipArchive::MovePackedFiles(DWORD uStartOffset,
							 DWORD uEndOffset,
							 DWORD uMoveBy,
							 CZipActionCallback* pCallback,
							 bool bForward)
