...
DWORD
ZipPlatform::GetDeviceFreeSpace(LPCTSTR lpszPath)
{
	...
	if (!GetDiskFreeSpace(szDrive,
						  ...
						  &TotalNumberOfClusters)) {
		CZipPathComponent::AppendSeparator(szDrive); // in spite of what is
													 // written in MSDN it is
													 // sometimes needed (on
													 // fixed disks)
		if (!GetDiskFreeSpace(szDrive,
							  &SectorsPerCluster,
...
bool
ZipPlatform::GetFileSize(LPCTSTR lpszFileName, DWORD& dSize)
{
	HANDLE f = CreateFile(lpszFileName,
						  GENERIC_READ,
						  FILE_SHARE_READ,
						  NULL,
						  OPEN_EXISTING,
...
CZipString
ZipPlatform::GetTmpFileName(LPCTSTR lpszPath, DWORD iSizeNeeded)
{
	TCHAR empty[] = _T("");
	CZipString tempPath;
	...
	if (bCheckTemp) {
		...
		if (GetDeviceFreeSpace(tempPath) < iSizeNeeded) {
			if (!GetCurrentDirectory(tempPath) ||
				GetDeviceFreeSpace(tempPath) < iSizeNeeded)
				return empty;
		}
	}
	CZipString tempName;
...
bool
ZipPlatform::GetCurrentDirectory(CZipString& sz)
{
...
bool
ZipPlatform::SetFileAttr(LPCTSTR lpFileName, DWORD uAttr)
{
	return ::SetFileAttributes(lpFileName, uAttr) != 0;
}

bool
ZipPlatform::GetFileAttr(LPCTSTR lpFileName, DWORD& uAttr)
{
...
bool
ZipPlatform::GetFileModTime(LPCTSTR lpFileName, time_t& ttime)
{
#if defined _MSC_VER && !defined __BORLANDC__
	struct _stat st;
...
bool
ZipPlatform::SetFileModTime(LPCTSTR lpFileName, time_t ttime)
{
	struct _utimbuf ub;
	ub.actime = time(NULL);
	ub.modtime = ttime == -1
				   ? time(NULL)
				   : ttime; // if wrong file time, set it to the current
	return _tutime(lpFileName, &ub) == 0;
}

bool
ZipPlatform::ChangeDirectory(LPCTSTR lpDirectory)
{
	return _tchdir(lpDirectory) == 0; // returns 0 if ok
}
int
ZipPlatform::FileExists(LPCTSTR lpszName)
{
...
ZIPINLINE bool
ZipPlatform::IsDriveRemovable(LPCTSTR lpszFilePath)
{
...
ZIPINLINE bool
ZipPlatform::SetVolLabel(LPCTSTR lpszPath, LPCTSTR lpszLabel)
{
	CZipPathComponent zpc(lpszPath);
	CZipString szDrive = zpc.GetFileDrive();
...
ZIPINLINE void
ZipPlatform::AnsiOem(CZipAutoBuffer& buffer, bool bAnsiToOem)
{
	if (bAnsiToOem)
...
ZIPINLINE bool
ZipPlatform::RemoveFile(LPCTSTR lpszFileName, bool bThrow)
{
...
ZIPINLINE bool
ZipPlatform::RenameFile(LPCTSTR lpszOldName, LPCTSTR lpszNewName, bool bThrow)
{
...
ZIPINLINE bool
ZipPlatform::IsDirectory(DWORD uAttr)
{
	return (uAttr & FILE_ATTRIBUTE_DIRECTORY) != 0;
}
ZIPINLINE bool
ZipPlatform::CreateDirectory(LPCTSTR lpDirectory)
{
	return ::CreateDirectory(lpDirectory, NULL) != 0;
}

ZIPINLINE DWORD
ZipPlatform::GetDefaultAttributes()
{
	return 0x81a40020; // make it readable under Unix
}

ZIPINLINE DWORD
ZipPlatform::GetDefaultDirAttributes()
{
...
#ifdef _UNICODE
int
ZipPlatform::WideToSingle(LPCTSTR lpWide, CZipAutoBuffer& szSingle)
{
	...
	int iLen =
	  WideCharToMultiByte(CP_ACP, 0, lpWide, wideLen, szSingle, 0, NULL, NULL);
	if (iLen > 0) {
		szSingle.Allocate(iLen, true);
		iLen = WideCharToMultiByte(
		  CP_ACP, 0, lpWide, wideLen, szSingle, iLen, NULL, NULL);
		ASSERT(iLen != 0);
...
int
ZipPlatform::SingleToWide(const CZipAutoBuffer& szSingle, CZipString& szWide)
{
	int singleLen = szSingle.GetSize();
	// iLen doesn't include terminating character
	int iLen = MultiByteToWideChar(
	  CP_ACP, MB_PRECOMPOSED, szSingle.GetBuffer(), singleLen, NULL, 0);
	if (iLen > 0) {
		iLen = MultiByteToWideChar(CP_ACP,
								   MB_PRECOMPOSED,
								   szSingle.GetBuffer(),
								   singleLen,
								   szWide.GetBuffer(iLen),
								   iLen);
...
#include <share.h>
bool
ZipPlatform::TruncateFile(int iDes, DWORD iSize)
{
...
int
ZipPlatform::OpenFile(LPCTSTR lpszFileName, UINT iMode, int iShareMode)
{
	switch (iShareMode) {
	...
	return _tsopen(lpszFileName,
				   iMode,
				   iShareMode,
				   S_IREAD | S_IWRITE /*required only when O_CREAT mode*/);
}

bool
ZipPlatform::FlushFile(int iDes)
{
	return _commit(iDes) == 0;
}

int
ZipPlatform::GetFileSystemHandle(int iDes)
{
