...
/**
	...
	( to set it to a specified value); setlocale() function is not sufficient
   here.
*/
...
class ZIP_API CZipString : public stdbs
{
	...
	static TCHAR tl(TCHAR c) { return std::tolower<TCHAR>(c, std::locale()); }
	static TCHAR tu(TCHAR c) { return std::toupper<TCHAR>(c, std::locale()); }
#else
	static int zslen(const TCHAR* lpsz)
	{
  ...
  public:
	...
	void Format(LPCTSTR lpszFormat, ...)
	{
		...
		do {
			...
			if (uTotal == -1 ||
				(uTotal == nLen - 1)) // for some implementations
			{
				pBuf[nLen - 1] = _T('\0');
				if (iCounter == 7)
					break;
	...
	{
#ifndef __GNUC__
		// use_facet doesn't work here well (doesn't convert all the local
		// characters properly)
	...
	bool operator!=(LPCTSTR lpsz) { return Compare(lpsz) != 0; }
	bool operator==(LPCTSTR lpsz) { return Compare(lpsz) == 0; }
	int Find(TCHAR ch, int nStart = 0) const { return (int)find(ch, nStart); }
