...
/**
	...
	and the offsets would be changed as well when the filename would changed its
   size)
*/
...
*/
	...
	/**
		\return
			get the effective compressed size: if the file is encrypted, return
	   less by the size of the encryption header
	*/
	...
	*/
	...
	{
		return m_uUncomprSize ? ((float)m_uComprSize) * 100 / m_uUncomprSize
							  : 0;
	}

	/**
	\return	\c if the file is encrypted ( a password is needed to extract
	this file) \see CZipArchive::SetPassword
*/
...
*/
	...
	/**
	\param bLocal
		if \c true return the local file header size or in the central directory
	otherwise \return	the total size of the structure depending on the \e
	bLocal parameter
*/
...
*/
	...
	/**
	\return	the attributes of the file converted to the current system
	compatible value \note
		- Throws exception if the archive system or the current system
	is not supported by the ZipArchive library.
		- (only Linux version): After obtaining the attributes, you need to
	shift them right by 16
*/
	...
	/**
		\return
		the original attributes as stored in the archive (without performing any
	   conversion)
	*/
	...
	If #m_uExternalAttr value is zero, the function check for a presence of a
	path separator at the end of the file name. If it is present, it is assumed
	to be a directory.
*/
...
*/
	...
	if you wish to set the attributes of this structure use
	CZipArchive::SetFileHeaderAttr() \param	uAttr attributes to set \note Throws
	exceptions if the archive system or the current system is not supported by
	the ZipArchive library. \see CZipArchive::SetFileHeaderAttr \see
	GetSystemAttr \see SetSystemCompatibility
*/
...
*/
	...
	\param	iLevel
		a compression level
	\param	bSpan
		\c true, if the data descriptor will be present because of spanning
	archive \param	bEncrypted \c true, if the file will be encrypted \return
	\c true if the sizes of the filename, extra field and comments does not
	exceed \c unsigned \c short maximum value; otherwise \c false;
*/
