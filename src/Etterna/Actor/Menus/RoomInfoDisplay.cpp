...
void
RoomInfoDisplay::RetractInfoBox()
{
	if (m_state == OPEN)
		OFF_COMMAND(this);
...
void
RoomInfoDisplay::SetRoom(const RoomWheelItemData* roomData)
{
...
void
RoomInfoDisplay::Update(float fDeltaTime)
{
	if ((m_deployDelay.PeekDeltaTime() >= DEPLOY_DELAY) &&
		(m_deployDelay.PeekDeltaTime() < (DEPLOY_DELAY + RETRACT_DELAY)))
		DeployInfoBox();
...
RoomInfoDisplay::SetRoomInfo(const RoomInfo& info)
{
	...
	m_players.SetText(ssprintf("%s (%d/%d):",
							   PLAYERS.GetValue().c_str(),
							   info.numPlayers,
							   info.maxPlayers));
