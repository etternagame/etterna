...
BPMDisplay::SetBPMRange(const DisplayBpms& bpms)
{
	...
	if (!(bool)CYCLE) {
		...
		int MaxBPM = INT_MIN;
		for (unsigned i = 0; i < BPMS.size(); ++i) {
			MinBPM = std::min(MinBPM, static_cast<int>(lround(BPMS[i])));
			MaxBPM = std::max(MaxBPM, static_cast<int>(lround(BPMS[i])));
		}
		if (MinBPM == MaxBPM) {
			if (MinBPM == -1)
		...
		m_iCurrentBPM = std::min<int>(1u, m_BPMS.size()); // start on the first hold
		m_fBPMFrom = BPMS[0];
		m_fBPMTo = BPMS[0];
		m_fPercentInState = 1;
	}
