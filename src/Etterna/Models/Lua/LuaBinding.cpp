...
RegisterTypes(lua_State* L)
{
	...
	while (!mapToRegister.empty()) {
		...
		for (;;) {
			...
			/* If the base class wasn't found, and hasn't been registered
			 * already, then a base class registration is missing. */
...
void
LuaBinding::Register(lua_State* L)
{
...
void
LuaBinding::CreateMethodsTable(lua_State* L, const std::string& sName)
{
	lua_newtable(L);
	lua_pushvalue(L, -1);
...
int
LuaBinding::PushEqual(lua_State* L)
{
 ...
 */
bool
LuaBinding::CheckLuaObjectType(lua_State* L,
							   int iArg,
 ...
 * the userdata table. */
void
LuaBinding::ApplyDerivedType(Lua* L, const std::string& sClassName, void* pSelf)
{
	int iTable = lua_gettop(L);

	int iType = lua_type(L, iTable);
	ASSERT_M(iType == LUA_TTABLE || iType == LUA_TUSERDATA,
			 ssprintf("Object on lua stack that derived type is being applied "
					  "to is %i instead of %i or %i",
					  iType,
					  LUA_TTABLE,
					  LUA_TUSERDATA));
...
LuaBinding::GetPointerFromStack(Lua* L, const std::string& sType, int iArg)
{
	...
	if (lua_istable(L, iArg)) {
		...
		if (lua_isnil(L, -1))
			luaL_error(L,
					   "stale %s referenced (object used but no longer exists)",
					   sType.c_str());
