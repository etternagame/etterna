...
int
putlocal(struct zlist* z, WRITEFUNC wfunc, void* param)
{
	// Write a local header described by *z to file *f.  Return a ZE_ error
	// code.
	PUTLG(LOCSIG, f);
	PUTSH(z->ver, f);
	PUTSH(z->lflg, f);
	...
	auto res = static_cast<size_t>(
	  wfunc(param, z->iname, static_cast<unsigned int>(z->nam)));
	if (res != z->nam)
		return ZE_TEMP;
	if (z->ext != 0u) {
		res = static_cast<size_t>(
		  wfunc(param, z->extra, static_cast<unsigned int>(z->ext)));
		if (res != z->ext)
			return ZE_TEMP;
	}
	return ZE_OK;
...
int
putextended(struct zlist* z, WRITEFUNC wfunc, void* param)
{
...
int
putcentral(struct zlist* z, WRITEFUNC wfunc, void* param)
{
	...
	PUTLG(z->atx, f);
	PUTLG(z->off, f);
	if (static_cast<size_t>(wfunc(
		  param, z->iname, static_cast<unsigned int>(z->nam))) != z->nam ||
		((z->cext != 0u) &&
		 static_cast<size_t>(wfunc(
		   param, z->cextra, static_cast<unsigned int>(z->cext))) != z->cext) ||
		((z->com != 0u) &&
		 static_cast<size_t>(wfunc(
		   param, z->comment, static_cast<unsigned int>(z->com))) != z->com))
		return ZE_TEMP;
	return ZE_OK;
}

int
putend(int n, ulg s, ulg c, size_t m, char* z, WRITEFUNC wfunc, void* param)
{
	...
	if ((m != 0u) && wfunc(param, z, static_cast<unsigned int>(m)) != m)
		return ZE_TEMP;
	return ZE_OK;
}
...
unsigned
TZip::sflush(void* param, const char* buf, unsigned* size)
{
	// static
	if (*size == 0)
		return 0;
...
unsigned
TZip::swrite(void* param, const char* buf, unsigned size)
{
	// static
	if (size == 0)
		return 0;
...
void
filetime2dosdatetime(const tm st,
...
unsigned
TZip::read(char* buf, unsigned size)
{
	if (bufin != nullptr) {
		if (posin >= lenin)
...
ZRESULT
TZip::Add(const TCHAR* odstzn, const TCHAR* src, unsigned long flags)
{
	if (oerr != 0u)
		return ZR_FAILED;
	if (hasputcen)
	...
	zfi.vem = static_cast<ush>(
	  0xB17); // 0xB00 is win32 os-code. 0x17 is 23 in decimal: zip 2.3
	zfi.ver = static_cast<ush>(20); // Needs PKUNZIP 2.0 to unzip it
	zfi.tim = timestamp;
	// Even though we write the header now, it will have to be rewritten, since
	// we don't know compressed size or crc.
	...
	zfi.siz = static_cast<ulg>(
	  method == STORE && isize >= 0 ? isize : 0); // to be updated later
	...
	writ += 4 + LOCHEAD + static_cast<unsigned int>(zfi.nam) +
			static_cast<unsigned int>(zfi.ext);
	if (oerr != ZR_OK) {
		iclose();
		return oerr;
	}
