...
TapNote TAP_EMPTY(TapNoteType_Empty,
				  ...
				  -1);
TapNote TAP_ORIGINAL_TAP(TapNoteType_Tap,
						 TapNoteSubType_Invalid,
						 TapNoteSource_Original,
						 -1);
TapNote TAP_ORIGINAL_LIFT(TapNoteType_Lift,
						  TapNoteSubType_Invalid,
						  TapNoteSource_Original,
						  -1);
TapNote TAP_ORIGINAL_HOLD_HEAD(TapNoteType_HoldHead,
							   TapNoteSubType_Hold,
							   TapNoteSource_Original,
							   -1);
TapNote TAP_ORIGINAL_ROLL_HEAD(TapNoteType_HoldHead,
							   TapNoteSubType_Roll,
							   TapNoteSource_Original,
							   -1);
TapNote TAP_ORIGINAL_MINE(TapNoteType_Mine,
						  TapNoteSubType_Invalid,
						  TapNoteSource_Original,
						  -1);
TapNote TAP_ORIGINAL_AUTO_KEYSOUND(TapNoteType_AutoKeysound,
								   TapNoteSubType_Invalid,
								   TapNoteSource_Original,
								   -1);
TapNote TAP_ORIGINAL_FAKE(TapNoteType_Fake,
						  TapNoteSubType_Invalid,
						  TapNoteSource_Original,
						  -1);
TapNote TAP_ADDITION_TAP(TapNoteType_Tap,
						 TapNoteSubType_Invalid,
						 TapNoteSource_Addition,
						 -1);
TapNote TAP_ADDITION_MINE(TapNoteType_Mine,
						  TapNoteSubType_Invalid,
						  TapNoteSource_Addition,
						  -1);

static const char* TapNoteTypeNames[] = {
	"Empty", "Tap",  "HoldHead",	 "HoldTail",
	"Mine",  "Lift", "AutoKeySound", "Fake",
};
...
GetNoteType(int row)
{
	...
	if (row % (ROWS_PER_MEASURE / 8) == 0)
		return NOTE_TYPE_8TH;
	if (row % (ROWS_PER_MEASURE / 12) == 0)
...
XNode*
TapNoteResult::CreateNode() const
{
...
void
TapNoteResult::LoadFromNode(const XNode* pNode)
{
	FAIL_M("TapNoteResult::LoadFromNode() is not implemented");
}

XNode*
HoldNoteResult::CreateNode() const
{
...
void
HoldNoteResult::LoadFromNode(const XNode* pNode)
{
	FAIL_M("HoldNoteResult::LoadFromNode() is not implemented");
}

XNode*
TapNote::CreateNode() const
{
...
void
TapNote::LoadFromNode(const XNode* pNode)
{
	FAIL_M("TapNote::LoadFromNode() is not implemented");
}

float
HoldNoteResult::GetLastHeldBeat() const
{
