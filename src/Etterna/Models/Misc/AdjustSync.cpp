	...
	switch (type) {
		...
		case AutosyncType_Tempo: {
			...
			s_vAutosyncTempoData.push_back(
			  make_pair(fStepTime, fNoteOffBySeconds));
			break;
		}
...
static LocalizedString TEMPO_SEGMENT_FROM("AdjustSync",
										  "%s BPM from %.3f BPM to %.3f BPM.");
...
#define SEGMENTS_MISMATCH_MESSAGE(orig, test, segments_name)                   \
	if ((orig).size() != (test).size()) {                                      \
		LuaHelpers::ReportScriptError("The sync overlay's " #segments_name     \
									  " segment list is a different size "     \
									  "from the song's.  Please report this "  \
									  "bug with steps to reproduce it.");      \
	}
		...
		for (size_t i = 0; i < stopTest.size() && i < stopOrig.size(); i++) {
			...
			RString s =
			  ssprintf(CHANGED_STOP.GetValue(), i + 1, fOld, fNew, fDelta);
			vsAddTo.push_back(s);
		}
		...
		if (vsAddTo.size() > iOriginalSize && s_iStepsFiltered > 0) {
			vsAddTo.push_back(
			  ssprintf(TAPS_IGNORED.GetValue(), s_iStepsFiltered));
		}
