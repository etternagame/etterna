...
std::map<int, ReplaySnapshot> PlayerAI::m_ReplaySnapshotMap;
std::map<float, std::vector<TapReplayResult>> PlayerAI::m_ReplayTapMapByElapsedTime;
std::map<float, std::vector<HoldReplayResult>> PlayerAI::m_ReplayHoldMapByElapsedTime;
						   ...
						   float timingScale)
{
    ...
    std::map<int, std::vector<HoldReplayResult>> m_unjudgedholds(m_ReplayHoldMap);
...
PlayerAI::GenerateComboListForReplay(float timingScale)
{
	...
	for (auto tapIter = m_ReplayTapMapByElapsedTime.begin();
		 tapIter != m_ReplayTapMapByElapsedTime.end();
		 tapIter++) {
        std::vector<TapReplayResult> trrv = tapIter->second;
