...
class PlayerAI
{
  public:
	...
	static std::map<float, std::vector<HoldReplayResult>> m_ReplayHoldMapByElapsedTime;
	...
	static std::pair<float, float> GetWifeScoreForRow(int row, float ts);

	// Given the Replay Data and Snapshot map, we can make a simple estimated
	// life graph.
	static std::map<float, float> GenerateLifeRecordForReplay(
	  float timingScale = Player::GetTimingWindowScale());

	// Given the Replay Data and Snapshot map, we can make a simple estimate
	// combo graph.
	static std::vector<PlayerStageStats::Combo_t> GenerateComboListForReplay(
	  float timingScale = Player::GetTimingWindowScale());
};
