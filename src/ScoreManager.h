...
public:
	...
	void LoadFromNode(const XNode* node, const string& key, const float& rate, const string& profileID);
...
public:
	...
	void LoadFromNode(const XNode* node, const string& ck, const string& profileID);
...
public:
	...
	vector<vector<HighScore*>> GetAllPBPtrs(const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	HighScore* GetChartPBAt(const string& ck, float& rate, const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	...
	int AddScore(const HighScore& hs_, const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID) { HighScore hs = hs_; RegisterScoreInProfile(pscores[profileID][hs.GetChartKey()].AddScore(hs), profileID); return hs.GetTopScore();}
	void ImportScore(const HighScore& hs_, const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	...
	void SortTopSSRPtrs(Skillset ss, const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	void RecalculateSSRs(LoadingWindow *ld, const string& profileID);
	void EnableAllScores();
	void CalcPlayerRating(float& prating, float* pskillsets, const string& profileID);
	...
	ScoresForChart* GetScoresForChart(const string& ck, const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	vector<string> GetSortedKeys();
	...
	void SetAllTopScores(const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
	void PurgeScores();
	unordered_map<string, ScoresForChart>* GetProfileScores(const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID) { return &(pscores[profileID]); };

	void PurgeProfileScores(const string& profileID = PROFILEMAN->GetProfile(PLAYER_1)->m_sProfileID);
private:
	unordered_map<string, unordered_map<string, ScoresForChart>> pscores;	// Profile scores
