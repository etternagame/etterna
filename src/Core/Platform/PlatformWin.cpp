...
// TODO: Consider *any* possibility where we don't need to access the registry.
// Works, but is it ideal?
std::string
RegistryGetString(HKEY hKey,
...
unsigned
RegistryGetDWORD(HKEY hKey, const std::string& subKey, const std::string& value)
{
	LONG retCode;					  // Return code for registry functions
...
getSystem()
{
	...
	std::string productName =
	  RegistryGetString(HKEY_LOCAL_MACHINE, versionKey, "EditionId");
	unsigned major = RegistryGetDWORD(
	  HKEY_LOCAL_MACHINE, versionKey, "CurrentMajorVersionNumber");
	unsigned minor = RegistryGetDWORD(
	  HKEY_LOCAL_MACHINE, versionKey, "CurrentMinorVersionNumber");
	std::string build =
	  RegistryGetString(HKEY_LOCAL_MACHINE, versionKey, "CurrentBuildNumber");

	return fmt::format(
	  "Windows {}.{} {} (Build #{})", major, minor, productName, build);
}
...
getSystemMemory()
{
	// Note: This assumes the user has the correct bit OS installed. If the user
	// has 16GB of ram, but instealled the 32bit operating system, the function
	// will only report 4gb of ram.
...
getSystemGPU()
{
	...
	std::string renderer =
	  (char*)glGetString(GL_RENDERER); // Returns a hint to the model
...
getLanguage()
{
	...
	GetUserPreferredUILanguages(
	  MUI_LANGUAGE_NAME, &numLanguages, nullptr, &languageBufferSize);

	// Then, put all those language codes in a buffer
	std::vector<wchar_t> languages(languageBufferSize + 1, 0);
	GetUserPreferredUILanguages(
	  MUI_LANGUAGE_NAME, &numLanguages, languages.data(), &languageBufferSize);
...
{
	// ShellExecuteA returns HINSTANCE for compatibility, and must be converted
	// into int.
...
openFolder(const ghc::filesystem::path& path)
{
	if (!ghc::filesystem::is_directory(path)) {
		Locator::getLogger()->warn(
		  "Could not open folder. Note a folder. Path: \"{}\"", path.string());
		return false;
	}
	ShellExecuteA(
	  nullptr, nullptr, path.native().c_str(), nullptr, nullptr, SW_SHOWNORMAL);
	return true;
}
...
getClipboard()
{
	...
	// Get text handler on clipboard, and prevent other applications from
	// accessing
