...
std::string
getX11UTF8String(Display* dpy, Window w, Atom p)
{
	Atom da, incr, type;
	int di;
	...
	if (type == incr)
		return ""; // printf("Data too large and INCR mechanism not
				   // implemented\n");
	...
	std::string res = reinterpret_cast<const char*>(prop_ret);
...
/* We can define these symbol to catch failed assert() calls.  This is only used
...
getSystem()
{
	...
	// We're reading /etc/os-release since a lot of distributions are using this
	// file.
	std::ifstream file{ "/etc/os-release" };
	while (std::getline(file, line)) { // Read line of cpu info
		auto delimLoc = line.find('=');
		auto label =
		  line.substr(0, delimLoc); // Get substring to only have key portion
		if (label.compare("PRETTY_NAME") != 0)
			continue; // If we have the wrong key, keep searching
...
getSystemCPU()
{
	...
	while (std::getline(file, line)) { // Read line of cpu info
		...
		auto label = line.substr(
		  0, delimLoc - 1); // Get substring to only have key portion
		if (label.compare("model name") != 0)
			continue; // If we have the wrong key, keep searching
...
getScreenDimensions()
{
	// Using X11 since it's currently a dependency, but is this the most
	// portable?
...
getWindowDimensions()
{
	auto width = static_cast<unsigned>(PREFSMAN->m_iDisplayHeight *
...
getLanguage()
{
	...
	if (langRes == nullptr) {
		...
		Locator::getLogger()->warn("Environment variable \"LANG\" not found. "
								   "Using \"en\" for language.");
		return "en";
	}
...
bool
openWebsite(const std::string& url)
{
	...
	while ((index = url_escaped.find('"', index)) != std::string::npos) {
		url_escaped.replace(
		  index, 1, "\\\""); // `1` is the length of the old `"`.
		index += 2;			 // `2` is the length of the new `\"`.
	}

	// xgd-open is the most portable command. Use in popular desktop
	// environments
	int resCode = system(fmt::format("xdg-open \"{}\"", url_escaped).c_str());
	if (resCode != 0) {
		Locator::getLogger()->warn(
		  "Unable to open url. xdg-open return code: {}. URL: {}",
		  resCode,
		  url);
		return false;
	}
...
bool
openFolder(const ghc::filesystem::path& path)
{
...
getClipboard()
{
	...
	if (display == nullptr) {
		Locator::getLogger()->warn(
		  "Couldn't access clipboard. Can't open X Display.");
		return "";
	}

	Atom clipboard = XInternAtom(
	  display, "CLIPBOARD", 0); // Get a refernece to clipboard selection
	Window owner = XGetSelectionOwner(
	  display, clipboard); // Get refernece to clipboard selection owner
	if (owner == None)
		return ""; // If there is no owner, there is no clipboard. (None is X11
				   // null constant)

	// Create a small window to paste clipboard into
	Window root = RootWindow(display, DefaultScreen(display));
	Window target_window =
	  XCreateSimpleWindow(display, root, -10, -10, 1, 1, 0, 0, 0);
	...
	XConvertSelection(
	  display, clipboard, utf8, target_property, target_window, CurrentTime);

	// Event loop to want and check to see if the other application has
	// convereted the stirng as requested. We get notified on the
	// SelectionNotify event. If it is successful, we run the function that
	// calls gets the X11 property. If it is not, we return an empty string.
	XEvent ev;
	XSelectionEvent* sev;
	while (true) {
		...
		switch (ev.type) {
			case SelectionNotify:
				...
				if (sev->property != None) {
					res =
					  getX11UTF8String(display, target_window, target_property);
					goto main; // Jump down to the outside of the infinite while
							   // loop.
				} else {
					return "";
				}
...
main: // Label to leave loop
	// At this point, we should have either returend an empty string, or
	// retrieved the properly convereted UTF-8 string.

	// Remove newline characters
	res.erase(std::remove(res.begin(), res.end(), '\n'),
			  res.end()); // Remove newlines
	res.erase(std::remove(res.begin(), res.end(), '\r'),
			  res.end()); // Remove carriage returns
...
bool
setClipboardText(std::string text)
{
	Locator::getLogger()->warn(
	  "Core::Platform::setClipboardText not implemented");
	return false;
}

void
setCursorVisible(bool value)
{
	...
	if (value) {
		...
		if (!pBlankCursor.has_value()) {
			...
			Pixmap BlankBitmap = XCreateBitmapFromData(
			  X11Helper::Dpy, X11Helper::Win, pBlank, 8, 8);
...
getExecutableDirectory()
{
	...
	int bytesRead = std::min<int>(
	  readlink("/proc/self/exe", locationBuffer, maxSize), maxSize - 1);

	// Null-terminate (readlink doesn't do this)
	if (bytesRead > 0)
		locationBuffer[bytesRead] = '\0';
...
bool
isOtherInstanceRunning(int argc, char** argv)
{
	Locator::getLogger()->warn(
	  "Core::Platform::isOtherInstanceRunning not implemented");
	return false;
}
