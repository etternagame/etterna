...
local t = Def.ActorFrame {
	LoadActor(Var "File1") .. { 
		OnCommand=function(self)
			self:x(scale(1,0,4,SCREEN_LEFT,SCREEN_RIGHT)):y(scale(1,0,4,SCREEN_TOP,SCREEN_BOTTOM)):cropto(SCREEN_WIDTH/2,SCREEN_HEIGHT/2):diffuse(Color1)
		end	
	};
	LoadActor(Var "File2") .. { 
		OnCommand=function(self)
			self:x(scale(3,0,4,SCREEN_LEFT,SCREEN_RIGHT)):y(scale(1,0,4,SCREEN_TOP,SCREEN_BOTTOM)):cropto(SCREEN_WIDTH/2,SCREEN_HEIGHT/2):diffuse(Color2)
		end	
	};
	LoadActor(Var "File2") .. {
		OnCommand=function(self)
